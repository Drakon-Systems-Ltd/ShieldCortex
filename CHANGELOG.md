...
- **Surfaces.** `threat_graph` MCP tool (`sources` / `source` / `events` / `allowances` views, row+byte capped, emergency-stop guarded); `shieldcortex threat-graph rebuild|status|reset-source` CLI (rebuild backfills from retained audit history); a `doctor` freshness check. New config block `threatGraph.{enabled, trustModifier, autoRelease}` — all default to the safe setting. New columns: `defence_audit.{source_attested, risk_modifier}`, `threat_edges.attrs`; new index `idx_audit_source_ident_ts`.
