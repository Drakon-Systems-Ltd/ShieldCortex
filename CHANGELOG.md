...
### Fixed
- **#444** Action Guard no longer hard-denies `python3 repair.py` because the script's docstring or comment quotes a global install in Markdown backticks. A bare backtick in the shell-out sink set was treated as command execution for every language, so a well-documented Python/JS script read as running the command it described. Backtick execution now counts only where the language runs it (Ruby, Perl, PHP) or where a JavaScript tagged template executes (structural scan: trivia, Unicode identifiers, member access). Nested tagged templates inside a bare interpolation remain a recorded gap. Nothing else in the classifier moved: real `os.system` / `subprocess` / `child_process` / `__import__('os').system` installs still gate. Guard stays off by default.
