...
- **#574 #576 an installer never writes through a symlinked destination.** Both installers overlay the packaged set file by file, and `mkdirSync`/`copyFileSync` follow a link at the destination — so a symlink at `plugins/shieldcortex/shadow.py`, at `hooks/cortex-memory/runtime.mjs`, or at the plugin directory itself made an install truncate a file elsewhere on the box. Every leaf about to be written is `lstat`ed and a link refuses that install, as do the path components between the integration root and the destination. A symlinked `~/.hermes` / `~/.openclaw` / profile root is refused by the lock's own preflight, before even the lock file is created.
