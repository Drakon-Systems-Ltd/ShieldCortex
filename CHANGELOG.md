...
- **#574 #576 a crash mid-swap is healed by reinstalling the package.** Replacing an installed directory is two `rename(2)` calls — `target → backups/`, then `staged → target` — and Node has no atomic directory exchange to do it in one (no `renameat2(RENAME_EXCHANGE)` binding). A SIGKILL, a power cut or an OOM between them left the host with NO installed copy, and the next `update` read that as "not installed" and walked past it. The packaged plugin and hook ship inside the npm package and are always correct for the installed version, so the recovery needs nothing else: when the STANDARD target has no `plugin.yaml` / `HOOK.md` **and** one of our own swaps left a `backups/<name>-preupdate-*` under that root, `update` stages the packaged tree and publishes it into that same standard path, and `doctor`'s **Hermes plugin** row WARNs with the command. Both inputs are computed — the target from the home Hermes' own discovery resolved (or the OpenClaw config root), the bytes from the package — so nothing on disk supplies a destination. The backup is evidence that this host had the integration installed, so a self-heal never conjures an install onto a host that never had one; it is never read, moved or deleted. A directory that merely exists is not an install, and a target that fails byte-verification is refreshed rather than skipped.
...
- **#574 #576 staged bytes are durable before they are reachable.** Every staged file is `fsync`ed, then every staged directory bottom-up, before the first rename makes the tree reachable; both parent directories are flushed after each rename. A file that will not flush refuses the publication outright — nothing has been written yet, so refusing costs a warning while publishing bytes that may not survive a power cut costs the gate. A platform that will not flush a directory fd (Windows, some filesystems) is reported in the step's detail rather than swallowed.
- **#574 #576 one writer per integration root.** `<hermesHome>/.shieldcortex-update.lock` and `<openclawRoot>/.shieldcortex-update.lock` (`O_CREAT|O_EXCL|O_NOFOLLOW`, holding the pid and a timestamp) are held by `update`'s two refresh steps and by `shieldcortex hermes install` / `shieldcortex openclaw install` while they write. A second command WARNs "another ShieldCortex update/install is running", names the lock and writes nothing, so two updates can never interleave their renames over one tree. A lock is only ignored when its pid is dead **and** it is more than ten minutes old — a run killed mid-swap therefore holds its own lock for up to ten minutes, which the refusal says out loud along with the path.
