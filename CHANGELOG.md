...
### Fixed
- **#519** Action Guard's global-install proposer no longer takes ~1 s on a command with thousands of repeated package-manager tokens. The proposer is now the package-manager token itself (linear); classification plus the argv disposer still decide, with a linear first-pm-then-scan confirm for interpreter-sink shapes the disposer does not parse. The disposer is confirm-only and never independently proposes, so a quoted `gh --body` mentioning an install stays a mention. Guard stays off by default.
