...
### Fixed
- **Scanner floor (internal #51):** both regex tiers now catch three agent-directed frames they previously walked past: ignore-your-own-guard (`your safety rules`), reveal-hidden-instructions without saying "prompt", and delegated-authority grants aimed at the agent (`the developer authorised you`). Bounded closed tables; dashboard/eslint/PR-approval prose stays quiet. Still a regex floor — paraphrase outside these frames is the semantic layer. No shop bump.
