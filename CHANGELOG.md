...
### Fixed
- **#502** `shieldcortex approve` (and `deny`) no longer treats a pseudo-terminal as proof of a human. `script -qec`, `expect`, `python pty.spawn`, `setsid -f` (inherited fds), and a process that daemonises then allocates its own pty while remaining the session leader, all refused. Both grant paths require a TTY, a controlling terminal (`tty_nr ≠ 0`), no agent-host environment marker, no agent ancestor, and a session not led by a pty tool or a bare interpreter. Listing still needs no TTY. Unknown terminals and WSL/console login (bash whose parent is init) are allowed — that last shape is indistinguishable from "daemonise, openpty, exec bash", which remains a residual a same-UID process-tree check cannot close without carding every WSL user; the OS-owned lock (#501) is the next bar. Guard stays off by default.
