...
### Fixed
- **Doctor NOTIFY:** signed Action Guard Enforce leftover no longer FAILs `NOTIFY` when the OpenClaw plugin explicitly has Guard off (`actionGuard.enabled:false` / `enforce:false`, or `interceptor.enabled:false`). That was Jarvis's 5.0.1 1-fail; the `$` footer prescribed a webhook. FAIL remains when a live enforcing plane claims a sink it does not have. Do not add a webhook. Guard stays off.
