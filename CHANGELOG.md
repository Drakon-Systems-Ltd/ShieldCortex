...
- **Hermes/REST DNP honesty (internal #63):** Hermes/REST privilege-escalation refusals no longer advertise a bare `shieldcortex approve` (held cards that this plane never creates). A DNP fingerprint is recorded and the reject text names the exact spendable `shieldcortex approve --denial <actionId>` command **once**: `POST /api/v1/action-guard` returns the id in `denial.actionId` and a semantic `reason`, and the Hermes policy is the single renderer of the command — previously both spelled it out and the block message named it twice. The reject copy stays bounded (reason clamped to one 400-char line, id regex-validated). The retry is session-bound (`bindSession`): host store + cwd + tool + host-provided session, never agent args. A grant minted for session A cannot be spent by session B. Catastrophic blocks still have no door. Cron `--denial` retries remain `{cwd, tool}` unless a lane opts into `bindSession`.
- **`shieldcortex approve --denial --any-origin` widens the DIRECTORY only (internal #63).** It never lifts a session binding: `grantRetry` keeps the recorded `sessionKey` on a widened grant and `consumeRetryGrant` still AND-matches tool + session when `anyOrigin` is set, so a grant minted for session A stays unspendable by session B in any directory. A session-bound fingerprint with no recorded key stays `unscopeable` rather than being widened into an unbound grant. The confirmation prompt and success copy now say "directory binding" and print the surviving session binding instead of promising "ANY local process". Ordinary cron `--denial` retries (no `bindSession`) are unchanged and still spend from any directory.
- **A REST retry grant that flips a refusal to an allow is now audited (internal #63).** `POST /api/v1/action-guard` writes a `defence_audit` ALLOW row before returning the allow, carrying the same evidence as the Claude Code hook lane's `approved` row: `[action-guard:retry_grant_consumed]` with tool, `action=require_approval`, `outcome=approved`, `grantKind=retry`, how the grant was minted (`via=card|tty`) and the `actionId`, bound to the call by its byte-exact `hashToolCall` digest in `content_hash`. No command text is stored. An unwritable ledger never un-spends the grant.
