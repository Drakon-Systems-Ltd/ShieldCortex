...
- **A declared `source` was a self-applied identity, and ownership was string equality against it (#283).** `resolveToolSource` only rejected a declaration that *outscored* the environment ceiling (#273) or tied it under a different name. Anything scoring **strictly below** the ceiling was honoured verbatim — unclamped, unaudited, `attested=false` — and then used as the `checkAccess` caller key, which keys ownership on `${type}:${identifier}`. On the four default `cli:*` 0.9 deployments (Claude Code CLI and IDE, Codex CLI and VS Code) a caller could declare `hook:session-end` (0.8) or `api:ingest` (0.7) and get **RESTRICTED owner-read** of the real holder's rows — credential-class cross-identity read, exactly what SCOPE P4 says is isolated — plus own-delete and trust-hierarchy revoke of every lower-trust source. `file:*` (0.6) got own-delete; `agent:*` (0.3) got INTERNAL own-read. The isolated-`HOME` case clamps all of it and looks closed; it inverts on the defaults, which is why it survived #269/#271/#273.

  The fix is one stamp, not a side-channel column: an identity the host did not attest is rewritten into a `claimed>` keyspace the host never mints, at the single point where it is resolved. That rewritten string is what gets stored on the row, what becomes the `checkAccess` caller key shared by read/delete/revoke, and what the read guards see — so a writer-chosen `hook:session-end` can no longer string-match the host-attested `hook:session-end` it is impersonating. `scoreSource` strips the marker before scoring, so the stamp moves **ownership only** and cannot raise trust (a naive prefix would have lifted `file:import` off its 0.4 quarantine pin to 0.6, and re-entered `agent` at the 0.5 `env-override>` pin). Hierarchy revoke — the one destructive cross-identity branch, granted purely on the caller's trust *number* and never consulting ownership — is gated on attestation directly. The `agent` exemption on the shared-context bootstrap now reads the stamp rather than the writer-chosen type, so an unattested `agent:` capture no longer bootstraps another agent's session while a real subagent's still does. Honoured-but-unconfirmed claims now write a `SOURCE_UNATTESTED_CLAIM` audit row instead of passing silently. `forget` stays env-inferred and is pinned by a structural test.

  The stamp is keyed on **environment confirmation**, not on the `attested` ledger bit. `deriveAttested` returns true whenever `strictSourceMode` is on — deliberately, because that bit rides to `defence_audit.source_attested` and gates threat-graph risk accrual, where strict is the operator's opt-in to hardened consequences. Keying the ownership rewrite on it meant strict mode was the one posture where the rewrite was skipped and every rung above kept the bare key: on a hardened `cli:mcp` 0.9 host, `hook:session-end` and `api:ingest` still had RESTRICTED owner-read, own-delete and hierarchy revoke, `file:notes` still had own-delete, and `agent:browser` still had INTERNAL owner-read. Ownership now keys on a separate `deriveEnvConfirmed` predicate that takes no posture input at all, so the two bits are free to disagree and the hardened deployment is no longer the weak one. The #283 matrix runs end to end on **both** postures.
