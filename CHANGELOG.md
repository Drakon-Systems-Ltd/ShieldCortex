...
### Fixed
- **#459:** doctor LOAD no longer reports UNPROTECTED when the running gateway listen line names `shieldcortex-realtime`. Live boot-roster proof (journalctl user unit, then `~/.openclaw/logs`, then `/tmp/openclaw`) outranks a `plugins_json` miss. Raw file sources require a parseable timestamp ≥ process start; only journalctl `--since` output may accept undated lines, so a stale undated `~/.openclaw/logs` roster cannot outrank a fresh dated `/tmp` roster. Conversation-hook consent remaining inactive is still a separate WARN. The #74 fail (readable index omit + no live roster) is unchanged.
