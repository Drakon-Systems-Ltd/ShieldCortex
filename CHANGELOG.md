...
- **#393 (Phase 1 — T1 host contract enforcement):** `sc_only` / `disable_native_inject` become bus laws doctor can prove, not labels it takes on trust.
  ...
  - Tests: `src/memory/__tests__/host-contract-393.test.ts` (35 cases — full runtime matrix, absent/unreadable/stale evidence, no-false-PASS sweep) and the T1 matrix added to `src/cli/__tests__/doctor-memory-plane-348.test.ts`.
