...
### Fixed
- **#569 doctor detects (and can move) shadowing Hermes plugin copies:** Hermes keys plugins on the manifest `name:` rather than the folder name, walks `$HERMES_HOME/plugins/` in sorted order, and on a same-source key collision the later manifest silently wins. A backup left beside the live plugin — `plugins/shieldcortex.bak-pre510-<ts>/`, the obvious thing to make before an upgrade — sorts after `plugins/shieldcortex/` and is what the gateway loads, so the upgrade installs new bytes and the gate keeps running the old code with nothing said about it (already seen in the field on the Ekho plugin through the same mechanism). `shieldcortex doctor` now has a `Hermes plugin copies` row backed by Hermes' own discovery (the two r2 entries below say how the answer is obtained) across `plugins/` and every `profiles/*/plugins/`, honouring `HERMES_HOME`. It WARNs (not fails) when there is more than one copy or when the loaded copy is not the canonical `plugins/shieldcortex`, naming every path and which one Hermes loads; an unreadable directory or a malformed manifest is skipped rather than thrown on. `doctor --fix-hermes-plugin-copies` MOVES each non-canonical copy into `<hermesHome>/backups/shieldcortex-shadow-<dirname>-<UTC ts>/`, never deletes, never overwrites an existing destination (it reserves a fresh one; see the r2 entry below), and leaves a root with no canonical copy entirely alone (which copy is authoritative is a human's call); it re-runs the check so the printed report is the post-fix state and says to restart the Hermes gateway. The plugin itself logs one ERROR line at `register()` when it is loaded from somewhere other than `shieldcortex/` or a sibling declares the same name, and `shieldcortex hermes install` warns about the same state instead of reporting a clean install.
- **#569 r2 the detector asks Hermes instead of imitating it:** the round-1 row mirrored Hermes' discovery rules in a line reader, and an independent review of the sibling Ekho fix found six shapes where a line reader and Hermes give different answers: `name: shieldcortex # backup` (Hermes parses YAML, so the comment is stripped and the backup IS a shadow), a manifest with no `name:` at all (Hermes falls back to the directory name, so the canonical copy was being dropped and the repair then refused to act for want of one), quoted names, a portable `plugin.json` package (Hermes accepts it), a `plugin.yaml` DIRECTORY beside a valid `plugin.yml` (Hermes selects on `exists()`, fails to parse, and takes nothing from that child — the line reader invented a shadow), and a valid `name:` line above broken YAML (Hermes rejects the manifest). Doctor now resolves the Hermes interpreter (`$HERMES_HOME/hermes-agent/.venv/bin/python3`, else the shebang of `hermes` on PATH), spawns it with no shell and a short timeout, and asks `hermes_cli.plugins_discovery` itself; the plugin's own start-up check runs inside Hermes and imports it directly. Where neither is reachable a conservative reader answers instead, every surface says `(approximate: Hermes discovery not reachable)`, and a manifest whose shape it does not model is reported `unknown` rather than guessed — a host with an unclassifiable manifest is no longer called clean.
- **#569 r2 the repair cannot overwrite or break the install:** `--fix-hermes-plugin-copies` now RESERVES its destination with an exclusive `mkdir` under `<hermesHome>/backups/` and moves the copy inside it, so a dangling destination symlink (absent to `existsSync`, present to `rename`) and a destination created between check and move both send the repair to a fresh name instead of through or over the occupant. On EXDEV it copies nothing and removes nothing: it reports that the copy is on another filesystem and must be moved by hand, and doctor exits non-zero. Before moving anything it refuses — non-zero — when the copy is a symlink, when the canonical `plugins/shieldcortex` is itself a symlink, or when the copy resolves into the same tree as the canonical install; the same physical directory seen through two plugin roots is moved once. Only a root with no canonical copy is still a quiet, exit-0 refusal. The plugin's start-up check derives its own directory from `__spec__.submodule_search_locations` without resolving symlinks, so a plugin loaded through `plugins/shieldcortex.bak-x -> /srv/sc-old` still reports the discovery path and names the canonical copy beside it.
- **#569 r3 the fallback reports "unknown" instead of guessing:** the round-2 fallback reader (used only where Hermes' own discovery cannot be reached) modelled as much of YAML as a line reader can and guessed at the rest, and review of the sibling Ekho change found the guesses wrong in both directions — `name: >-` with an indented `shieldcortex` under it made it report a clean install while Hermes was loading that backup, and `description: backup: before upgrade` under a valid `name:` line made it label a backup LOADED that Hermes rejects. A manifest is now UNDERSTOOD only when every non-blank, non-comment line is a column-0 `key: value` whose value is a plain or quoted single-line scalar with no unquoted `: ` in it, or an indented continuation of a key other than `name`; block scalars, flow collections, anchors, aliases, tags, a null `name:`, document markers, tabs and any directory whose effective manifest is a portable `plugin.json` are all **unknown**. One unknown costs its whole root its verdict: the row cannot PASS for that root, no winner is named, `--fix-hermes-plugin-copies` moves nothing there, and the plugin's own start-up line says it could not determine (WARNING) rather than staying silent. `unknown` is reserved for manifests that could still take our key — the directory is named `shieldcortex`, the text mentions it, or the text carries a backslash — so a neighbouring third-party manifest with a `description: >` in it does not put a permanent "cannot determine" on a healthy host. Differential fixtures in both languages assert the fallback gives the same answer as Hermes or unknown, never a confident disagreement.
- **#569 r3 the repair preflights every plugin root before it moves anything:** the round-2 refusals compared a candidate only against its own root's canonical copy, which cannot see `profiles/work/plugins/shieldcortex` pointing at `plugins/shieldcortex.bak-x`. The main root then looks like two ordinary directories, the repair moves the backup, that profile's install is left dangling — and because the profile's own scan had one canonical copy and was skipped as clean, a re-run reports PASS on a host whose plugin has just been taken away. `--fix-hermes-plugin-copies` now resolves every directory Hermes could use across ALL roots (main plus every `profiles/*/plugins`, canonical copies in otherwise clean roots included) and abandons the whole repair — non-zero, naming the dependent path — if any candidate is one of them, contains one, or lives inside one. A profile root that is itself a symlink to the main one still moves the shared directory exactly once.
- **#569 r3 honesty and bounds in the small:** the Hermes log suppression is scoped to the discovery loggers (`hermes_cli.plugins*`, `agent_plugins`, and whatever `logger` those modules actually hold), never the root logger, and in-process it also only swallows records from the scanning thread, so a concurrent gateway warning still gets through. The real reason the fallback was reached is kept and printed on every surface (`no Hermes interpreter found`, the spawn error, or `hermes_cli discovery raised <ExcType>`) instead of being flattened to "not importable". A reserved destination left behind by a failed move is released with an empty-directory-only removal, and the residual race (another writer inside our freshly created container) is documented rather than escalated to a recursive one. The plugin's start-up check declines a `plugins/` root holding more than 256 entries and says so at DEBUG; `doctor` has no such cap, because there the scan is the work.
- **#569 r4 no verdict without Hermes' own discovery:** rounds 1 to 3 each shipped a reader of our own for hosts where the Hermes interpreter cannot be reached, narrowing its grammar every round; four rounds of independent review on the sibling Ekho change found a confident wrong answer in every version of it, and the last two needed no exotic syntax at all — `description: 2026-99-99` (YAML types the plain scalar as a timestamp, month 99 fails construction, and Hermes drops the whole manifest, where the grammar had accepted the line) and `manifest_version: .inf` (YAML constructs infinity and Hermes' `int()` conversion raises `OverflowError`). That meaning lives in YAML's implicit typing and in Hermes' own conversion code, so the chase has no end, and a row whose entire job is to be right about what the gateway loads cannot ship "probably". **The reader is gone in both languages.** Without Hermes' discovery the `Hermes plugin copies` row is always **WARN** — never PASS, never a winner — reading `could not determine which copy Hermes loads (<the real reason>). Install Hermes, or point the doctor at its python.`; it may list directories whose folder name starts with `shieldcortex` or whose manifest text contains that substring as `possible copies (unverified)`, with no manifest read and nothing counted as a copy. `--fix-hermes-plugin-copies` then moves nothing and exits non-zero. The plugin's own start-up check runs inside Hermes, so it uses `hermes_cli` only: if the import or discovery ever fails there it logs one DEBUG line and returns, with no ERROR, no WARNING and nothing that reads as a verdict. `shieldcortex hermes install` warns only from Hermes' discovery and is silent when it is unavailable — an install log is the wrong place to learn "I could not tell".
- **#569 r4 the repair refuses symlinked layouts and preflights the whole plan:** comparing final `realpath`s misses a chain through an intermediate link — `profiles/work/plugins/shieldcortex -> plugins/shieldcortex.bak-x/forward -> /srv/elsewhere`, where the endpoints never overlap yet moving the backup takes `forward` with it and leaves the profile's install dangling. So the repair no longer models where each link goes: if **any** copy Hermes discovered in **any** root is a symlink or holds one anywhere in its tree, the whole repair stops. That walk follows nothing, is bounded at 20 000 entries, and refuses on an unreadable directory or an exhausted budget — "I did not finish looking" is not "there is nothing there". And executing root by root left later roots preflighting a tree that earlier moves had already changed (the main root's backup moves, the profile's walk then meets a missing directory and refuses), so the repair now builds the complete plan across all roots from ONE scan, preflights that plan against that same scan, and executes only if every item passed: any hard refusal abandons the repair in every root. A root with no canonical `plugins/shieldcortex` remains the one quiet, exit-0 refusal, and does not stop the other roots. The atomic `mkdir` destination reservation and the EXDEV refusal are unchanged.
- **#569 r5 the plugin ROOTS come from Hermes too:** asking Hermes' own discovery about a directory we worked out ourselves is not parity, and review of the sibling Ekho change found both ways that goes wrong. `HERMES_HOME=$HOME/.hermes` (the literal string, which a service unit or a shell profile leaves behind easily) is expanded by Hermes — `hermes_constants._expand_hermes_home` is `expanduser(expandvars(path))` — so the gateway loads the backup under the real directory while we scanned a path that does not exist, found nothing, and reported **PASS**. And `HERMES_HOME=<root>/profiles/work` means `get_default_hermes_root()` hands Hermes back `<root>`, so `<root>/plugins` and every SIBLING profile are live plugin roots; looking for profiles under the ACTIVE home instead, a repair run from `work` never saw `<root>/plugins/shieldcortex -> profiles/work/plugins/shieldcortex.bak-x` and moved the directory that link depends on, leaving the default profile's install dangling. The embedded Hermes probe now calls `hermes_constants.get_hermes_home()` and `get_default_hermes_root()` with the doctor's `HOME`/`HERMES_HOME` passed through **unexpanded**, builds the protective root set there (`<root>/plugins`, every `<root>/profiles/<name>/plugins`, and the active home's `plugins` when that did not already cover it), and returns the active home, the containing root and the per-root copies and winner. Nothing in TypeScript computes a root for a verdict or a move any more — the symlink walk, the cross-root preflight, the plan and `<hermesHome>/backups/` all use Hermes' answer — and the surviving TS resolution is documented as hint-list scaffolding that decides nothing. If `hermes_constants` is missing or either function raises, the result is **undetermined** exactly as an unavailable discovery is: WARN, no winner, and a repair that moves nothing and exits non-zero. Every root is reported on, with the active one labelled, so a shadow in a sibling profile is visible from whichever profile the doctor runs under.
- **#569 r5 a root nobody can repair stops every root:** with canonical + backup in the main root and only `shieldcortex.bak-x` in a profile, the profile contributed a refusal and the main root's backup still moved — after the row had just said a human must choose which copy in that profile is authoritative, and the copy they choose can be the one another root resolves through. A root that holds a copy but no canonical `plugins/shieldcortex` now refuses the WHOLE plan: nothing moves in any root, and the refusal names the root to resolve first. It stays a quiet, exit-0 refusal when those copies were all the command had to do; when it abandons moves that were ready to go, those moves did not happen and the exit code says so.
