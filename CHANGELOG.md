...
### Fixed
- **#444** Action Guard no longer hard-denies `python3 repair.py` because the script's Python docstring or comment quotes a global install in Markdown backticks. A bare backtick in the shell-out sink set was treated as command execution for every language. It now counts only where the language can execute it (Ruby, Perl, PHP, and JavaScript tagged templates -- JavaScript keeps its previous any-backtick behaviour until a real lexer lands). Real `os.system` / `subprocess` / `child_process` / `__import__('os').system` installs still gate. Guard stays off by default.
