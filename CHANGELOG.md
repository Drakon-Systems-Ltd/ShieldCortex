...
- #510: PII identifiers are redacted on memory write, not just flagged at audit time. UK NI numbers, US SSNs, labelled tax ids (UTR/EIN/TIN) and salary figures become `[REDACTED:<kind>]` in the stored title, content, FTS index and sync payload; email/phone go too when the same memory holds an identifier (a bare vendor contact stays readable). Such content is classified at least CONFIDENTIAL with `pii:*` labels. PII never blocks a write. `SHIELDCORTEX_PII_REDACTION=off` stores verbatim but keeps the labels. Tags and metadata are redacted too: a metadata key that names an identifier (`utr`, `ssn`, `ni`, `tax_id`, `salary`, `pay`, …) redacts its value whatever its type, `Date`/`Buffer`/`Map`/`Set` values keep their type, and a metadata subtree past the scan bounds (depth 8 / 5,000 containers) is replaced with `"[REDACTED:unscanned]"` rather than stored unscanned — the row becomes CONFIDENTIAL. The write-audit `content_hash` of a PII-bearing record is taken over the redacted text (no guessing oracle); every other audit hash is byte-identical to before. Under an identifier key a string holding any digit is replaced whole (a redaction token already in the value exempts nothing), binary values are replaced too, and the key's kind is not inherited into descriptor children (`year`, `date`, `period`, `frequency`, `currency`, `band`, `grade`, `type`, `id`, `note(s)`, `source`, `updated`, `created`) — `{salary:{amount, year, currency}}` loses only `amount`. Redacted rows are never merge/consolidation candidates. On the hook write path a redacted candidate is a duplicate only when the project already holds a row with the identical redacted title and content created in the last 24 hours, or already holds 3 such rows of any age: a re-extraction loop stores one row, and two people's records captured more than 24 hours apart are both kept. Legacy migration keeps a row's embedding unless its title or content changed. An identifier held only in tags or metadata counts for the audit hash too, and an identifier redacted by an earlier write still marks the record, so a contact detail added by a later partial update is redacted beside it. A merge keeps the highest sensitivity either row held — redaction only raises a row to CONFIDENTIAL and never lowers a RESTRICTED one. Hook-written `session_events` (prompt-recall, session-end, pre-compact; single and batch) and memory-file scan findings queued into `quarantine` go through the same redactor; a previously reviewed quarantine row stored raw re-queues once.
  ...
  - Residuals, by design: (1) an unlabelled lowercase `ab123456c`, unlabelled `AB123456a` and unlabelled undashed `078051120` are NOT redacted — without a label they are indistinguishable from hex digests and ids; all three ARE redacted when labelled (NI / SSN context). (2) The hook write paths (memories and `session_events`) load the redactor from `dist`; if it is missing or stale — or throws — the memory or event is still stored — UNREDACTED, at CONFIDENTIAL or above (a RESTRICTED or SECRET label is kept, never lowered), with one stderr notice per hook process — because dropping memories is worse. This is a fail-safe, not a fail-closed redaction guarantee. Run `shieldcortex doctor` after an upgrade so `dist` is fresh. (3) Names and postal addresses are not detected. (4) Metadata nested deeper than 8 levels is replaced by the `unscanned` token even when it held no PII.
