...
- **Scanner floor (internal #51, PR #462):** both regex tiers now share bounded frames for three agent-directed themes: ignore-your-own-guard (`your safety rules`), reveal-hidden-instructions without saying "prompt", and delegated/claimed authority grants aimed at the agent (`the developer authorised you`).
  ...
  - Punctuation policy is rule-specific: own-rules and authority retain the #204 punctuation-run fold (`Ignore,,, your safety rules`); extraction never erases `...` to join separate sentences. Legacy #204/#318 rules keep their existing fold. Shared Unicode preprocessing avoids refolding the source per policy, with at most three variants per rule.
  - Still a regex floor: gerunds, ambiguous `reset`, nested directives and broader paraphrase/provenance distinctions remain outside these new frames. The existing input sanitiser converts U+2028/U+2029 to spaces before scan-only; raw detector line fidelity does not extend through that conversion. No shop bump.
