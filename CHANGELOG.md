...
- **Hermes/REST DNP honesty (internal #63):** Hermes/REST privilege-escalation refusals no longer advertise a bare `shieldcortex approve` (held cards that this plane never creates). A DNP fingerprint is recorded and the reject text names the exact spendable `shieldcortex approve --denial <actionId>` command. The retry is session-bound (`bindSession`): host store + cwd + tool + host-provided session, never agent args. A grant minted for session A cannot be spent by session B. Catastrophic blocks still have no door. Cron `--denial` retries remain `{cwd, tool}` unless a lane opts into `bindSession`.
