...
### Fixed
- **#550 session lease:** (a) A shell command maps to the `security-config` lease scope only when its shape WRITES a protected file (`~/.shieldcortex/config.json`, `~/.claude/settings.json`, `~/.openclaw/openclaw.json`): a redirect onto it, a mutating verb given it as an operand (`tee`, `sed -i`, `perl -i`, `rm`, `mv`, `truncate`, `chmod`/`chown`, `touch`, `dd of=`, an editor), a copy-like verb with it as the destination (`cp`, `install`, `ln`, `rsync`, `scp`), or an interpreter/shell handed it anywhere on the surface (`python3 -c`, `node -e`, a heredoc into `python3 -`, `bash -c`, `python3 script.py <file>` — the mapper does not parse programs, so these fail closed). Before, the three path rows fired on the path anywhere in the command text, so a commit message, a heredoc string literal, a `grep` pattern or a `cat` took the lease. Statements are split quote- and heredoc-aware: a heredoc body is data belonging to the stage that opened it (`cat > /tmp/x <<'EOF'` … a line naming the file … `EOF` takes nothing; the statements after the terminator are still judged), and a `;`, `|` or `>` inside a quoted string is not a separator or a redirect. A redirect glued to the word before it (`printf x>~/.openclaw/openclaw.json`) is a redirect; a redirect written inside a heredoc body is not (the body is data, read only by the interpreter rule). In-place flags are read with their quotes off and inside clusters (`sed "-i"`, `sed -ni`, `sed -Ei`, `--in-place[=suffix]`, `perl -i.bak`). An output option naming the file is a write whatever the verb (`git diff --output=<file>`, `--output <file>`, `-o <file>`). Reads (`cat`, `jq`, `diff`, `ls`, `sed -n`, `git log --`), backups (`cp <file> <file>.bak`, `cp <file> /tmp/…`, `tar`, `rsync <file> host:`) and mentions take no lease. Write/Edit tool calls are judged on their target path exactly as before. (b) Two enforcement planes gate one tool call under two identities when OpenClaw runs Claude Code as a child: the interceptor first, as the OpenClaw session id (record pid = the gateway), then the PreToolUse hook, as the hashed Claude session id — which found a "foreign" live holder and refused, naming the caller's own session, the gateway pid and `0s ago`; the refusal left the gateway's ten-minute record in place, so every retry was refused again. The hook now passes `spawnedRuntimeReentry`, and a live lease whose recorded pid is the process that spawned the hook's harness — its parent, or its grandparent through exactly one intermediate (gateway → claude → hook) — re-enters without acquiring, refreshing or releasing anything: the runtime that holds it already gated this very call under its own identity. Depth is capped at two so a nested `claude -p` under a Bash tool (depth three or more), or `bash -c 'exec claude …'` (depth three), cannot inherit a lease the gateway holds for a different session; pid ≤ 1, the hook's own pid and an unreadable process chain never qualify; a dead holder pid is still reaped by #438 first. The flag is opt-in and off for the interceptor, `evaluateAction` and every host adapter, which keep strict identity matching. A refused check writes nothing to the lease record (pinned byte-for-byte). Disclosed costs: an interpreter given the file as a read-only argument (`node cli.js doctor --config <file>`) takes a re-entrant lease it did not need; a script that writes the file without naming it on the command line (`python3 patch.py`, `openclaw config set`) is invisible to the mapper as before.
