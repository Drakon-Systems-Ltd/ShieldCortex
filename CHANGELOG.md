...
### Fixed
- **#550 session lease:** (a) A shell command maps to the `security-config` lease scope only when its shape WRITES a protected file (`~/.shieldcortex/config.json`, `~/.claude/settings.json`, `~/.openclaw/openclaw.json`): a redirect onto it, a mutating verb given it as an operand (`tee`, `sed -i`, `perl -i`, `rm`, `mv`, `truncate`, `chmod`/`chown`, `touch`, `dd of=`, an editor), a copy-like verb with it as the destination (`cp`, `install`, `ln`, `rsync`, `scp`), or an interpreter/shell handed it anywhere on the surface (`python3 -c`, `node -e`, a heredoc into `python3 -`, `bash -c`, `python3 script.py <file>` — the mapper does not parse programs, so these fail closed). Before, the three path rows fired on the path anywhere in the command text, so a commit message, a heredoc string literal, a `grep` pattern or a `cat` took the lease. Statements are split quote- and heredoc-aware: a heredoc body is data belonging to the stage that opened it (`cat > /tmp/x <<'EOF'` … a line naming the file … `EOF` takes nothing; the statements after the terminator are still judged), and a `;`, `|` or `>` inside a quoted string is not a separator or a redirect. The mapper FAILS CLOSED: a command that names a protected file on its executable surface takes the lease unless its shape is a positively proven read or mention — the file inside a quoted string, a comment or a heredoc body; a redirect elsewhere; a verb known to only read its operands (`cat`, `jq`, `diff`, `grep`, `ls`, `stat`, `head`, `tail`, `wc`, checksums, `echo`, `printf`, `awk` and `sed` without an in-place flag, `tar` create/list, `git` log/diff/show/status/blame/add/commit/push and other non-working-tree subcommands, `dd if=`); the source side of a copy. An unknown verb given the file (`curl -o`, `wget -O`, `patch`, `yq -i`, `awk -i inplace`, `npx json -I -f`, `git checkout --`/`git restore`) is a write until proven otherwise, and so is a file handed over through `xargs`, an assignment whose value is the file (`T=<file>; echo x > "$T"`), or a redirect target computed by substitution. A redirect glued to the word before it (`printf x>~/.openclaw/openclaw.json`) is a redirect; a redirect written inside a heredoc body is not. In-place flags are read with their quotes off and inside clusters (`sed "-i"`, `sed -ni`, `sed -Ei`, `-i.bak`, `--in-place[=suffix]`). An output option naming the file is a write whatever the verb (`git diff --output=<file>`, `curl -o`, `wget -O`). Statements are read through compound keywords (`if … then touch <file>; fi`), wrappers with option arguments (`sudo -u root tee`, `env -u VAR touch`, `nice -n 10 rm`, `timeout 5 tee`), a newline inside quotes (a multi-line `python3 -c` program is one statement), and a `<<` inside quotes, a comment or `$(( ))` opens no heredoc, so the lines after it are still judged. Reads (`cat`, `jq`, `diff`, `ls`, `sed -n`, `git log --`), backups (`cp <file> <file>.bak`, `cp <file> /tmp/…`, `tar`, `rsync <file> host:`) and mentions take no lease. Write/Edit tool calls are judged on their target path exactly as before. (b) Two enforcement planes gate one tool call under two identities when OpenClaw runs Claude Code as a child: the interceptor first, as the OpenClaw session id (record pid = the gateway), then the PreToolUse hook, as the hashed Claude session id — which found a "foreign" live holder and refused, naming the caller's own session, the gateway pid and `0s ago`; the refusal left the gateway's ten-minute record in place, so every retry was refused again. The hook now passes `spawnedRuntimeReentry`, and a live lease re-enters without acquiring, refreshing or releasing anything when BOTH hold: its recorded pid is the process that spawned the hook's harness — its parent, or its grandparent through exactly one intermediate (gateway → claude → hook) — AND the record was taken for this very call: every acquisition stamps `gatedCall` (a key over the scope and the call surface — the command text or the target path, identical on both planes — and the time), and the hook's call must carry the same key within 60 seconds. Ancestry alone is not identity: one gateway process backs every session's record, so a sibling session's hook shares the parent; the call key tells the calls apart, and a record without one (an older writer) never re-enters. A lost acquisition race is re-decided with the same facts asked of the winner's record. Depth is capped at two so a nested `claude -p` under a Bash tool (depth three or more), or `bash -c 'exec claude …'` (depth three), cannot inherit a lease the gateway holds for a different session; pid ≤ 1, the hook's own pid and an unreadable process chain never qualify; a dead holder pid is still reaped by #438 first. The flag is opt-in and off for the interceptor, `evaluateAction` and every host adapter, which keep strict identity matching. A refused check writes nothing to the lease record (pinned byte-for-byte). Disclosed costs: an interpreter or shell given the file as a read-only argument (`node cli.js doctor --config <file>`, `bash -c 'cat <file>'`, `perl -ne print <file>`) and an unknown read-only tool given the file take a lease they did not need — the fail-closed direction; a script that writes the file without naming it on the command line (`python3 patch.py`, `openclaw config set`, `find … -delete` on the directory) is invisible to the mapper as before. Residual: a hook-only call (one the interceptor never saw) that repeats, within 60 seconds, the exact command another session's gateway record was taken for re-enters that record; the interceptor plane refuses such a call first whenever it sees it.
