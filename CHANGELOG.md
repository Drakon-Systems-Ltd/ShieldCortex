...
- **`GET /api/graph/paths` is bounded and scoped**: BFS stops at 2000 visited entities / 500 edges per node and reports `truncated: true` (so an empty `path` with `truncated` means "not found within budget", not "no path exists"); `?project=` scopes both endpoints, every hop and `sourceMemories`; each hop gains `entityType`, `memoryCount`, `confidence`, `disputed`.
