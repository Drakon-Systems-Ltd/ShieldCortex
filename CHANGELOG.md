...
### Fixed
- **#471:** `shieldcortex doctor` no longer SIGABRTs at process teardown on Node 24 (`Database::~Database` → `Assertion failed: (env) != nullptr`, exit 134). Same better-sqlite3 12 ObjectWrap class as the worker crash; the doctor now closes the singleton before returning so GC does not run the destructor after the Node environment is gone, and the live CLI spawn no longer treats a signal death as exit 0.
