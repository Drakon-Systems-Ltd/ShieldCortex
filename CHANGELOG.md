...
### Fixed
- **#444** Action Guard no longer hard-denies `python3 repair.py` because the script's docstring or comment quotes a global install in Markdown backticks. A bare backtick was treated as a shell-out sink for every language, so well-documented Python/JS repair scripts read as executing the command they described. Backtick execution now counts only where the language runs it (Ruby, Perl); the exec path applies the same invocation disposer the write path already used. Real `os.system` / `subprocess` / `child_process` installs still gate. Guard stays off by default.
