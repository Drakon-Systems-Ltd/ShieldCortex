...
### Added
- **#573 `shieldcortex logs prune`, plus the same bound automatically:** retention for `~/.shieldcortex/logs/project-key-repair-*.json`, the one on-disk plane nothing ever deleted (3,508 files on the reporting host, peak 357 in a day). Keeps the newest 20 (`SHIELDCORTEX_REPAIR_LOG_KEEP`, a whole number of at least 1 — anything else is refused with a warning and the default used). Dry-run by default, `--execute` to act; the background worker applies the same bound once per ~24h. Only that exact name, only directly in the logs directory, only regular files by `lstat`, re-checked immediately before the `unlink` — no compression, no rename, no temporary file. A logs directory reachable through a symlink refuses the whole pass and says so.
...
- **#573 the repair log follows the database it describes:** `memories repair-project-keys --execute` wrote its per-rewrite JSON log to `os.homedir()/.shieldcortex/logs/` no matter which database it had repaired, while the safety backup beside it correctly followed the DB. Every run against a throwaway database — a test, a probe, `--db <tmp>` — therefore left a permanent file in the operator's real home describing a database that no longer existed; all 3,508 on the reporting host were of that kind, and not one recorded a real repair. The log is now written to `<db-dir>/logs/`, which for the default DB is exactly the documented `~/.shieldcortex/logs/`.
- **#573 doctor's DISK advice names the real consumer, and never guesses destructively:** the breakdown now splits the `logs` lump into `audit` / `repair` / `other` and reports the DB's free pages (`freelist_count × page_size`), and the remedy addresses whichever term is actually largest. Memory-deletion advice (`memories prune` / `dedupe`) requires positive page-level attribution — the pages of `memories` plus its indexes and FTS shadow tables, read from `dbstat`, at least half the database's used pages, with the DB the largest term. No `dbstat`, an unreadable DB, or a minority share all get inspection (`shieldcortex stats`, plus `vacuum` when free pages are over 25%) instead. The live incident read `DB 54.2 MB · logs 41.4 MB` and offered only memory deletion.
