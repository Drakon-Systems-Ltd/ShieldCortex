...
- **#574 #576 the directory swap is crash-safe.** Replacing an installed directory is two `rename(2)` calls — `target → backups/`, then `staged → target` — and Node has no atomic directory exchange to do it in one (no `renameat2(RENAME_EXCHANGE)` binding). A SIGKILL, a power cut or an OOM between them left the host with NO installed copy, and the next `update` read that as "not installed" and walked past it. Both refreshes now write a durable journal (`<hermesHome>/.shieldcortex-refresh-journal.json`, `<configRoot>/.shieldcortex-refresh-journal.json` — exclusive create, `fsync`ed, outside every discovery root) before the first rename, and delete it last. `shieldcortex update`, `shieldcortex hermes install` and `shieldcortex openclaw install` all begin by finishing an interrupted swap, and `doctor`'s **Hermes plugin** row WARNs while one is unresolved. Recovery is decided by what is on disk, never by the recorded phase: target present → clear the journal and keep the backup; target missing with the backup there → restore the previous copy, which is the one the host was demonstrably running; target and backup both gone → publish the staged copy, but only after it verifies byte-for-byte against the package. Anything it cannot read refuses and leaves the journal, the staging tree and the backup exactly where they are, with the recovery command printed.
- **#574 #576 the whole write path is preflighted for symlinks.** The round-1 preflight walked the CONTENTS of every discovered plugin directory (the #569 question) and never lstat'd the directories being written THROUGH, so a symlinked `<hermesHome>/backups`, a symlinked plugins root or a symlinked `HERMES_HOME` was followed. Every component of every write path — the plugins root and its ancestors up to the Hermes tree, the target, `backups/`, the staging parent and the journal's directory — is now checked before the first write, and staging and backups are proved to be outside EVERY root the probe returned rather than just the one being refreshed. The OpenClaw side checks `hooks/`, the hook directory and `backups/` the same way. A link, or a component that cannot be read, writes nothing. The `--fix-hermes-plugin-copies` repair semantics are unchanged.
