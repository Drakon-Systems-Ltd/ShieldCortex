...
- **#543** A provider key split by separators — `sk-T3st K3yA bCdE …`, or the same value broken by a tab, newline, NBSP, soft hyphen, zero-width space/joiner, word joiner or BOM — is now detected, blocked and redacted; on 5.0.7 the split variants produced zero findings, and removing the separators rebuilt the identical value. The scanner runs the provider patterns a second time over a separator-collapsed view that remembers where each character came from, and acts on the ORIGINAL span: the finding's `position` is the key's first character and the redaction covers the key and the inserted separators. Such findings carry `evasion: 'separator_split'`. Patterns that consult whitespace themselves (`KEY=…` assignments, `Authorization: Bearer …`, private-key blocks, connection strings) are not run on the collapsed view. A collapsed hit only counts when the value holds both a letter and a digit, so shouted or hyphenated prose that collapses into a key shape (`ASIA PACIFIC REGIONAL SALES`, `key-value pairs are stored …`) does not fire; a word before or after a key (`npm_… end`) is trimmed off the span, and a direct-pass match marks where a token begins so `sk-… and ghp_…` stays two keys. A year or a quarter gives prose that digit (`ASIA 2026 REGIONAL SALES REPORT`, `sk-proj- keys replaced the legacy format in 2024`), so a collapsed hit is also dismissed when it is structurally a heading or sentence. For open-ended or mixed-case patterns (`sk-…`, `ghp_…`, `AIza…`): every fragment between separators is either a word (three or more letters, one case or Capitalised, no letter pair English does not use; or a two-letter function word such as `by`, `in`, `to`) or a period token (`2026`, `7`, `Q1`, `H2`, `FY26`, `W12`, `1H`, `1st`), at least one word of three or more letters sits beyond the pattern's literal prefix, and words make up half the hit or more; one fragment that is neither (`4K2M`, a lone letter, mixed case) makes the hit a key, so the decision cannot be diluted by filler around a split key. For a fixed-length single-case pattern (the AWS access key id, `A[KS]IA` + 16 capitals or digits) that rule had to decide whether `GDP`, `PROJECT` or `HEADCOUNT` is a word, and headings such as `ASIA Q1 GDP GROWTH FORECAST` or `ASIA 2026 CUSTOMER ACQUISITION REPORT` were blocked as critical. Such a pattern cuts a window of fixed width out of any shouted heading, so the window itself is judged instead: a hit that starts or ends inside a letters-only fragment (`FOREC|AST`, `EUR|ASIA`) is a heading cut mid-word, and an aligned hit counts only when some fragment beyond the prefix is key material — letters and digits mixed, punctuation, mixed case, a digit run that is no period token — or the id was split into single characters. A whitespace-split AWS id must also respect the base-32 alphabet of issued ids (`[A-Z2-7]`: no 0, 1, 8 or 9), a fact stated on the pattern as `collapsedValuePattern` and consulted by the collapsed pass only; the direct pass on contiguous text is unchanged. The word at either end of a hit is the alphanumeric run around the boundary, not the whole run between separators, so a heading keeps its verdict when it is quoted, bold, bracketed, ends in `:` or `,`, or sits in a JSON string (`"ASIA 2026 REGIONAL SALES REPORT"`, `**…**` and `{"title": "…"}` were blocked while the bare heading was clean). The #543 test generates 6,000 headings (all-caps, Title and lower case) from ~150 report words, abbreviations, years and period tokens behind `ASIA`/`AKIA`, and scans each bare and wrapped thirteen ways (78,000 texts); the collapsed pass fires on none of them. Where the direct pass already matched a value starting at the same character, the collapsed pass leaves it alone and resumes after it: an identifier or a second, split token after a contiguous key is no longer swallowed into it (`sk-… customer123` redacts the key only). Trimming a word off the end (or start) of a hit stops at the innermost word of the trailing run: one- and two-letter fragments inward of it are the key's own tail and stay inside the finding, and a trim is only accepted when the pattern still matches what is left, so trimming never removes a finding. Candidates are discovered from every pattern first and then resolved: a candidate that contains another pattern's candidate is cut at that start when the cut span still validates (`sk_test_… AIza…` is a Stripe key and a Google key, both reported), and a candidate is only ever dropped when an already accepted range of equal or higher severity covers it whole — a lower-severity finding never hides a higher one.
  - Residuals, measured on random synthetic keys: a key with no digit at all is not matched by the collapsed pass (about 4% of AWS access key ids; negligible for mixed-case bodies). Against an attacker who chooses the split points optimally (`scripts/lab/credential-split-evasion.mts`: exact search over every partition plus a letter glued onto either end, 20,000 random ids per class, each predicted evasion re-run through the scanner), the strict rule dismisses none of 20,000 `sk-` or `AIza` bodies. For AWS ids the alignment rule is weaker by design: a naive split — after every 1, 4, 5 or 8 characters, once in the middle, or across newlines — is found for 100% / 99.9% / 99.99% / 99.96% / 100% / 99.9% of 19,312 random ids that hold a digit (after every 2: 89.7%; every 3: 98.3%); those rates now hold whether the split id is followed by a space, a comma or a closing quote — before the boundary-word change a trailing comma glued onto the last fragment made it key material by accident, so comma-delimited every-4 splits were found for 100% of 20,000 ids and are now found for 99.89%, the same as with spaces alone (treating the delimiter as evidence recovers them but fires on 1,564 of the 78,000 wrapped headings, so it is not done) — but an attacker who picks the split points evades it for 94.2% of ids with whitespace alone and for 100% with one letter glued onto the id. The strict rule would hold that to 1.7%, at the price of blocking about one all-caps heading in ten (205 of the 2,000 in the generated corpus); the collapsed pass therefore catches an AWS id that was wrapped, tabulated or spaced out, not one hidden by an adversary who knows this rule. A key split after a head long enough to match on its own is reported as an ordinary contiguous finding (no `evasion` flag) and only the head is redacted; the tail that remains is not a usable credential. The exact-span check is compiled once per pattern and looks back at most 256 characters, and a hit is trimmed with a bounded number of attempts, so scanning a split key followed by 16,000 words takes about 20 ms (was 1.3 s). Candidate resolution bisects candidates sorted by start and answers "is this span already covered at this severity" from a per-severity prefix-maximum index, and the redacted text is built in one pass instead of one splice per finding, so 1k / 2k / 4k / 8k split AWS ids scan in 14 / 25 / 47 / 92 ms (was 23 / 120 / 393 / 2404 ms). The git-SHA/UUID allowlist check stops expanding a token once it is longer than anything the allowlist accepts (64 characters), so split ids separated by spaces alone — one alphanumeric run in the collapsed view — no longer cost a walk over the whole input per hit: 800 / 1600 / 3200 such ids scan in 14 / 30 / 46 ms (was 415 / 1704 / 6551 ms).
  - Residuals, by design: a 1–2 letter fragment directly after a finely split key cannot be told from the key's tail and is included in the span (over-redaction; the finding is unaffected). A collapsed hit that could not be cut at an inner token start stays whole, so the inner key is reported twice (once on its own, once inside the wider span) rather than hidden. Visible punctuation (`sk-abc.def`) is not a separator. A fixed-length pattern with a trailing lookahead (Mailgun `key-…`, the 32-hex Azure rule) still needs a non-alphanumeric character after the value once the separators are gone. A heading whose abbreviation carries a digit (`B2B`, `3PL`) is key material by the alignment rule and still fires when the AWS window lands on it; a heading that starts a word with `ASIA`/`AKIA` in lower or Title case never matched. An AWS id written with a 0, 1, 8 or 9 (a documentation sample, not an issued id) is still caught contiguous but not split.
