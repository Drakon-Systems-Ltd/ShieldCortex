...
### Fixed
- **#519** Action Guard's global-install proposer no longer takes ~1 s on a command with thousands of repeated package-manager tokens. The statement-wide skip after each `npm`/`yarn`/`pnpm`/`bun` token was unbounded, so every later token restarted a scan to end-of-statement (O(n²)). The skip is now 512 characters; padded-flag exec (verb past that window) is still gated by the existing argv walk on the raw command. Folded file bytes are not walked (that would re-break #444). Guard stays off by default.
