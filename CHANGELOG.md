...
### Fixed
- **#471:** `shieldcortex doctor` no longer SIGABRTs at process teardown on Node 24 (`Database::~Database` → `Assertion failed: (env) != nullptr`, exit 134). The cause is the same better-sqlite3 12 `node::ObjectWrap` cleanup as the worker crash, and the fix is the move to the 13.x line recorded under *Changed* above — the doctor's own code is unchanged, because it never owned the connection that aborted (every DB check opens its own short-lived read-only handle). What is new here is the regression guard: the live-CLI test now builds a real initialised database in an isolated `HOME` before spawning `shieldcortex doctor`, so the destructor actually runs, and asserts the child died of no signal, exited 0, and printed no `Assertion failed: (env) != nullptr`. That spawn helper previously mapped a `null` exit code to 0, which would have scored a SIGABRT death as a green doctor run.
