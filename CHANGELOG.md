...
- **#543** A provider key split by separators — `sk-T3st K3yA bCdE …`, or the same value broken by a tab, newline, NBSP, soft hyphen, zero-width space/joiner, word joiner or BOM — is now detected, blocked and redacted; on 5.0.7 the split variants produced zero findings, and removing the separators rebuilt the identical value. The scanner runs the provider patterns a second time over a separator-collapsed view that remembers where each character came from, and acts on the ORIGINAL span: the finding's `position` is the key's first character and the redaction covers the key and the inserted separators. Such findings carry `evasion: 'separator_split'`. Patterns that consult whitespace themselves (`KEY=…` assignments, `Authorization: Bearer …`, private-key blocks, connection strings) are not run on the collapsed view. A collapsed hit only counts when the value holds both a letter and a digit, so shouted or hyphenated prose that collapses into a key shape (`ASIA PACIFIC REGIONAL SALES`, `key-value pairs are stored …`) does not fire; a word before or after a key (`npm_… end`) is trimmed off the span, and a direct-pass match marks where a token begins so `sk-… and ghp_…` stays two keys. A year or a quarter gives prose that digit (`ASIA 2026 REGIONAL SALES REPORT`, `sk-proj- keys replaced the legacy format in 2024`), so a collapsed hit is also dismissed when 60% or more of it sits in fragments that read as English words (three or more letters, one case or Capitalised, no letter pair English does not use). Where the direct pass already matched a value starting at the same character, the collapsed pass leaves it alone and resumes after it: an identifier or a second, split token after a contiguous key is no longer swallowed into it (`sk-… customer123` redacts the key only).
  - Residuals, measured on random synthetic keys: a key with no digit at all is not matched by the collapsed pass (about 4% of AWS access key ids; negligible for mixed-case bodies). The word gate costs a further 3–5% of split AWS ids, and an attacker who chooses the split points can get roughly one AWS id in eight past the collapsed pass; for the mixed-case formats (OpenAI, GitHub and the like) the miss rate is 0.3% or less. A key split after a head long enough to match on its own is reported as an ordinary contiguous finding (no `evasion` flag) and only the head is redacted; the tail that remains is not a usable credential. No collapsed-pass finding on 16 MB of Markdown prose, as written and upper-cased.
  - Residuals, by design: the attacker picks the split points, so a trailing run of same-case alphabetic fragments that holds a 3+ letter word (`… WxY zzz`) is trimmed as far as the pattern allows and can leave the key's last letters outside the redaction span — the finding still blocks. A 1–2 letter word directly after a finely split key is included in the span (over-redaction). Visible punctuation (`sk-abc.def`) is not a separator. A fixed-length pattern with a trailing lookahead (Mailgun `key-…`, the 32-hex Azure rule) still needs a non-alphanumeric character after the value once the separators are gone. Access-key ids with no digit at all (about 4% of 16-character base-32 bodies) are not caught by the collapsed pass.
