...
- **#573 the repair log follows the database it describes, and is settled before the repair runs:** `memories repair-project-keys --execute` wrote its per-rewrite JSON log to `os.homedir()/.shieldcortex/logs/` no matter which database it had repaired, while the safety backup beside it correctly followed the DB. Every run against a throwaway database — a test, a probe, `--db <tmp>` — therefore left a permanent file in the operator's real home describing a database that no longer existed; all 3,508 on the reporting host were of that kind, and not one recorded a real repair. The log is now written to `<db-dir>/logs/`, which for the default DB is exactly the documented `~/.shieldcortex/logs/`. That destination is also resolved, created and proved writable **before** the database is modified: a regular file or a symlink at `<db-dir>/logs` used to throw with the rewrite already committed and no log anywhere, and a `logs` pointed at the realtime audit directory made the repair write into the audit plane. A refusal now means the repair never ran and nothing changed. The record itself is created with `O_CREAT|O_EXCL`, so a preplanted symlink carrying its name is never followed — that shape previously overwrote a realtime audit file with repair JSON. If the write fails after the commit (disk full), the error names the row count, the database and the backup path, and exits non-zero; it is never silent.
- **#573 doctor's DISK advice names the real consumer, and never guesses destructively:** the breakdown now splits the `logs` lump into `audit` / `repair` / `other` and reports the DB's free pages (`freelist_count × page_size`), and the remedy addresses whichever term is actually largest. The live incident read `DB 54.2 MB · logs 41.4 MB` and offered only memory deletion. A consumer is named as "the bulk" only when it is measured to be one — at least 40% of the live DB file and at least half of everything measured — so a database holding one byte of `defence_audit` and 500 KB of threat-graph nodes is no longer reported as audit-dominated; when nothing clears that bar doctor says there is no single measured consumer and asks you to inspect (`doctor --verbose`, `stats`) rather than naming a destructive command. Memory-deletion advice (`memories prune` / `dedupe`) additionally requires positive page-level attribution from `dbstat`: the pages of `memories`, its indexes, the real FTS5 virtual table (identified from its `CREATE VIRTUAL TABLE … USING fts5` statement) and that table's five shadow tables by exact name — an ordinary table merely *called* `memories_fts_backup` is no longer counted as the search index, and no SQL `LIKE` name pattern is involved anywhere. `vacuum` is recommended only when measured free pages are at least 20% of the file, and the old blanket "these five directories are safe to rotate or clear" instruction — which covered `state/` and `quarantine/`, and was offered to a directory holding one state file and no database — is gone.
