...
/**
 * #283/#287 — the strict-mode divergence, end to end through the projector.
 *
 ...
 * rewrites the env-unconfirmed identity into the `claimed>` keyspace BEFORE it
 ...
 */
describe('#287 strict-mode accrual lands on the stamped key, not the victim bare key', () => {
  ...
  it('stamps the strict-mode claim before it reaches the ledger', () => {
    ...
    expect(resolved.source.identifier).toBe('claimed>session-end');
  ...
  it('projects the attacker onto a node disjoint from the victim', () => {
    ...
    expect(keys).toContain('hook:claimed>session-end');
  ...
  it('accrues decayed risk on the stamped key and leaves the victim key at none', () => {
    ...
    const stamped = rows.find(r => r.source_key === 'hook:claimed>session-end');
