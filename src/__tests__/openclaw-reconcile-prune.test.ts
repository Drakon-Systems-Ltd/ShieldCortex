...
function setupTwoDirHost(opts: {
  ...
  db.prepare(`INSERT INTO installed_plugin_index VALUES ('k',1,'v','x',1,'h',1,'r',@ir,@pj,'[]',NULL,1)`).run({
    ...
    // OpenClaw requires enabled/origin/rootDir; the default rootDir sits outside
    // npm/projects so only an explicit rootDir can name the live dir.
    pj: JSON.stringify(opts.rosterEntries.map((e) => ({ origin: 'global', rootDir: `/fixture/extensions/${String(e.pluginId)}`, ...e }))),
  });
  db.close();
}
