...
export const SAFE_CORPUS: GuardCorpusEntry[] = [
  ...
  //
  // HONESTY PIN (#596): this row's key set must EQUAL the measured 2026.8.1
  // revision in `tool-input-schema.ts` — no more, no less. A key the host never
  // declared is silently dropped as contract drift and the row stays green, so
  // the label would overclaim without the pin; `guard-precision-corpus.test.ts`
  // enforces it. `category` (an internal gateway mapping) and `timeoutSeconds`
  // (another tool's field) were exactly that kind of stowaway (#595 review).
  { tool: 'sessions_spawn', args: { task: 'inspect tests', taskName: 'review_tests', label: 'review', runtime: 'subagent', agentId: 'edith', model: 'default', runTimeoutSeconds: 60, thinking: 'medium', cwd: '/workspace', thread: true, mode: 'run', cleanup: 'delete', sandbox: 'inherit', context: 'bounded', lightContext: true, collect: true, outputSchema: { type: 'object', properties: { verdict: { type: 'string' } }, required: ['verdict'] }, fastMode: 'auto', groupId: 'swarm-1', visible: true, group: 'Reviews', worktree: true, worktreeName: 'wt-review', worktreeBaseRef: 'main', attachments: [{ name: 'notes.txt', content: 'hello', encoding: 'utf8' }], attachAs: { mountPath: '/mnt/attachments' }, resumeSessionId: 'sess-1', streamTo: 'parent' }, expect: 'allow', why: 'live OpenClaw sessions_spawn contract — all 28 declared fields' },
