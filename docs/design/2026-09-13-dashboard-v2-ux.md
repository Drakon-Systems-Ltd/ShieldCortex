...
### 14.a Fix round (review of the built result, 13 Sep 2026)

10. **Memory created-date range filter in Focus (§6.3, review item 12) — not implemented.** The neighbourhood payload's memories are server-capped (`memLimit`, default 40, by salience), so a client-side date filter would only filter the already-cut top-40 and would silently show "no memories in range" when in-range memories exist below the cap. Doing it honestly needs a server-side `createdAfter`/`createdBefore` on `/api/graph/entities/:id/neighbourhood` (and `/memories`) applied *before* the cap, plus a `counts.totalMemories` that respects it — a small API addition, but out of this round's scope. The other two §6.3/§7 items from that review item (context menu; conversation-scanning row) are implemented.
11. **"Conversation scanning" row on the Overview Protection tile (§7) mirrors the Injection Scanner module, not an independent switch.** `/api/iron-dome/status` exposes no separate conversation/prompt-scanning toggle (checked `src/defence/iron-dome/index.ts` `getIronDomeStatus` and `config.ts`); the Injection Scanner runs whenever Iron Dome is enabled, which is exactly how Protection → Status derives that module's state. The row therefore reads `on (injection scanner)` / `off` from `enabled`, says so in its label, and goes through the same pending/unavailable/stale derivation as every other row. Inventing a distinct on/off would have been a fabricated state.
12. **`/api/graph/paths` now answers `400` (not `404`) for a malformed `fromId`/`toId`** such as `12junk` — one strict positive-integer id parser is shared by every `:id`/`fromId`/`toId` (review item 3). `404` remains the answer for a well-formed id that does not exist (or is out of the requested project scope). Recorded in the CHANGELOG *Changed* section.
13. **`/api/graph/entities/:id/triples` is now bounded (default 200, max 1000)** where it used to return every live triple. Additive `total`/`limit`/`hasMore` fields let a caller detect the cut. Recorded in the CHANGELOG.
14. **`src/__tests__/upgrading-5-notice.test.ts` asserted the CHANGELOG's Unreleased section still read "(none yet)".** That was a release-time snapshot that fails the moment any post-5.0.0 entry lands — it was already failing at `d0214880` (the step-7 CHANGELOG entry removed the placeholder), which means the step-7 report's "root `npm test` 8490/8497, 553/553 suites" claim could not have been observed on that tree. The assertion now checks the section's shape (a "(none yet)" placeholder or at least one entry line); the ordering assertions around the 5.0.0 Breaking notice are untouched.
