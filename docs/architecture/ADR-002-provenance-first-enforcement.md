...
- **Section 5C — spawn-contract drift measured at a host version (round 3 follow-up).** OpenClaw 2026.9.5 (ec9c1a1) declares five `sessions_spawn` fields the reviewed `openclaw.sessions_spawn` contract does not carry, so ordinary coordinator dispatch logs `CONTRACT DRIFT` on every call. Recorded as a coverage-and-noise finding with three requirements — per-version schema tracking, a pinned quiet-dispatch fixture, unknown-field reporting preserved — explicitly **not** a bypass.
...
**Contract drift on the spawn contract, measured at OpenClaw 2026.9.5 (ec9c1a1).** A coverage-and-noise finding, not a bypass. The host's `sessions_spawn` input schema (`dist/sessions-spawn-tool-CE1WiS6Q.mjs`) declares `expectsCompletionMessage: Type.Optional(Type.Boolean(...))`, and alongside it `completionTarget`, `group`, `projectId` and `projectGitUrl`; the reviewed contract `openclaw.sessions_spawn` (`OPENCLAW_SPAWN_KEYS` in `src/defence/iron-dome/tool-input-schema.ts`) carries none of the five, and still carries `category`, which this host version no longer declares. None of the five sits in `GUARD_EVIDENCE_KEYS`, so each is dropped before nested validation and before any extractor, the call proceeds, and the key name is returned in `strippedKeys`: no evidence-bearing field is lost and no verdict widens. The cost is signal quality. Ordinary coordinator dispatch sets `expectsCompletionMessage`, so **every** such spawn writes `action-guard CONTRACT DRIFT <tool> (openclaw.sessions_spawn): dropped unread field(s) expectsCompletionMessage` at WARNING on the allow path (`plugins/openclaw/interceptor.ts`). A drift channel that fires on every routine call is a channel the operator learns to ignore, which is precisely how the next host schema move goes unnoticed. Some builds inject the field internally rather than taking it from the caller, so "the model sent it" does not separate the cases; the observation has to be read from the host's declared schema, per version. Three 5C requirements follow. **(1)** The contract tracks the host's declared schema **per host version**, measured against a recorded version string rather than hand-widened once more — that would be the fourth widening in the series after #436, #445 and the 13-of-28 round. **(2)** Ordinary coordinator dispatch is pinned as a fixture asserting zero dropped keys at the pinned version, so routine traffic is quiet and a real schema move is loud. **(3)** Unknown-field reporting is preserved: suppressing the warning, or allow-listing a name without measuring the host, is not the fix, and a dropped field never acquires semantics by being dropped quietly.
