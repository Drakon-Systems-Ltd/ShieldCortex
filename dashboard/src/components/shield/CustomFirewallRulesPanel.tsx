    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-[var(--sc-text)]">Custom Firewall Rules</h3>
          <span className="text-xs text-[var(--sc-text-muted)]">{rules.length}/25</span>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-1 px-2 py-1 text-xs bg-[var(--sc-ok)]/20 text-[var(--sc-ok)] rounded hover:bg-[var(--sc-ok)]/30 transition-colors"
        >
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="p-3 bg-[var(--sc-surface-2)]/50 rounded-lg space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Rule name" required className="px-2 py-1.5 text-xs bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded text-[var(--sc-text)] placeholder:text-[var(--sc-text-muted)]" />
            <input value={form.priority} onChange={e => setForm(f => ({ ...f, priority: Number(e.target.value) }))} type="number" placeholder="Priority" className="px-2 py-1.5 text-xs bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded text-[var(--sc-text)]" />
          </div>
          <div className="grid grid-cols-3 gap-2">
            <select value={form.condition_type} onChange={e => setForm(f => ({ ...f, condition_type: e.target.value }))} className="px-2 py-1.5 text-xs bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded text-[var(--sc-text)]">
              <option value="regex">Regex</option>
              <option value="source">Source</option>
              <option value="keyword">Keyword</option>
            </select>
            <input value={form.condition_value} onChange={e => setForm(f => ({ ...f, condition_value: e.target.value }))} placeholder="Condition value" required className="px-2 py-1.5 text-xs bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded text-[var(--sc-text)] placeholder:text-[var(--sc-text-muted)]" />
            <select value={form.action} onChange={e => setForm(f => ({ ...f, action: e.target.value as 'block' | 'allow' | 'quarantine' }))} className="px-2 py-1.5 text-xs bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded text-[var(--sc-text)]">
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={createRule.isPending} className="px-3 py-1 text-xs bg-[var(--sc-ok)] text-[var(--sc-text)] rounded hover:bg-[var(--sc-ok)]/80 disabled:opacity-50">
          ...
          {createRule.error && <p className="text-xs text-[var(--sc-danger)]">{(createRule.error as Error).message}</p>}
        </form>
      )}
          ...
          {rules.map(rule => (
            <div key={rule.id} className="flex items-center gap-3 px-3 py-2 bg-[var(--sc-surface-2)]/30 rounded text-xs">
              ...
              <button
                ...
                className="text-[var(--sc-text-muted)] hover:text-[var(--sc-text)]"
                title={rule.enabled ? 'Disable' : 'Enable'}
              >
                {rule.enabled ? <ToggleRight size={14} className="text-[var(--sc-ok)]" /> : <ToggleLeft size={14} />}
              </button>
              <button onClick={() => deleteRule.mutate(rule.id)} className="text-[var(--sc-text-muted)] hover:text-[var(--sc-danger)]">
                <Trash2 size={12} />
              </button>
            </div>
          ))}
...
export function CustomFirewallRulesPanel() {
  return (
    <div className="bg-[var(--sc-surface)]/50 border border-[var(--sc-border)] rounded-xl p-4">
      <RulesTable />
    </div>
  );
}
